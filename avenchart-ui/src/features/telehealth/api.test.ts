...
describe('telehealth transport boundaries', () => {
  ...
  it('fast-tracks only the signed-in patient\'s current synthetic request into the physician demo queue', async () => {
    sessionStorage.setItem('avenchart-ui.portalSession', JSON.stringify({ sessionId: 'portal-session', username: 'patient', portalUsername: 'patient', displayName: 'Synthetic Patient' }))
    fetchMock.mockResolvedValue(jsonResponse({ requestId: 'request-1', status: 'Queued' }))

    await fastTrackPatientRequestToQueue('request/1', 7)

    const [url, init] = fetchMock.mock.calls[0]
    const headers = new Headers(init?.headers)
    expect(String(url)).toContain('/patient/requests/request%2F1/demo-queue')
    expect(init?.method).toBe('POST')
    expect(headers.get('X-AvenChart-Patient-Portal-Session')).toBe('portal-session')
    expect(headers.get('X-Idempotency-Key')).toBe('00000000-0000-4000-8000-000000000001')
    expect(JSON.parse(String(init?.body))).toEqual({ expectedVersion: 7, syntheticDemoConfirmed: true })
  })

  it('submits applicant queued withdrawal only with the access key, current request version, confirmation, and idempotency key', async () => {
    fetchMock.mockResolvedValue(jsonResponse({ requestId: 'request-1', requestStatus: 'Cancelled' }))

    await withdrawApplicantTelehealthRequest('applicant/1', 'applicant-key', 'request/1', 13, 'withdrawal-key')

    const [url, init] = fetchMock.mock.calls[0]
    const headers = new Headers(init?.headers)
    expect(String(url)).toContain('/applicants/applicant%2F1/telehealth-request/request%2F1/withdraw')
    expect(init?.method).toBe('POST')
    expect(headers.get('X-AvenChart-Telehealth-Applicant-Key')).toBe('applicant-key')
    expect(headers.get('X-Idempotency-Key')).toBe('withdrawal-key')
    expect(headers.get('X-AvenChart-Patient-Portal-Session')).toBeNull()
    expect(JSON.parse(String(init?.body))).toEqual({ expectedRequestVersion: 13, syntheticWithdrawalConfirmed: true })
  })
  ...
  it('recovers only the authenticated physician\'s active synthetic work without a mutation key', async () => {
    sessionStorage.setItem('avenchart-ui.clinicianSession', JSON.stringify({ sessionId: 'physician-session', username: 'doctor', displayName: 'Doctor', role: 'provider', facilityId: 10, purposeOfUse: 'treatment' }))
    fetchMock.mockResolvedValue(jsonResponse({ shift: { shiftId: 'shift-1', status: 'Active' }, reservation: null }))

    await getClinicianActiveWork()

    const [url, init] = fetchMock.mock.calls[0]
    const headers = new Headers(init?.headers)
    expect(String(url)).toContain('/clinician/active-work')
    expect(init?.method).toBeUndefined()
    expect(init?.cache).toBe('no-store')
