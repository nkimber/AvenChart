...
function Invoke-Status([string]$Method,[string]$Path,[hashtable]$Headers=@{},[object]$Body=$null){
  $parameters=@{Uri="$ApiBaseUrl$Path";Method=$Method;Headers=$Headers;TimeoutSec=20;UseBasicParsing=$true}
  if($null-ne$Body){$parameters.ContentType='application/json';$parameters.Body=$Body|ConvertTo-Json -Depth 8}
  try {
    $response=Invoke-WebRequest @parameters
    if($null-eq$response){return 200}
...
try {
  ...
  Add-Check 'Prescription signing rejects an absent staff identity' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/prescription' @{} @{expectedDraftVersion=1;noCurrentMedicationsConfirmed=$true;noKnownAllergiesConfirmed=$true;adequateEvaluationConfirmed=$true;syntheticDataConfirmed=$true}) -eq 401)
  Add-Check 'Final clinical-review workspace rejects an absent staff identity' ((Invoke-Status 'GET' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/final-clinical-review') -eq 401)
  Add-Check 'Final clinical-review record rejects an absent staff identity' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/final-clinical-review' @{} @{expectedDocumentationVersion=1;expectedDispositionVersion=1;documentationReviewed=$true;physicianResponsibilityConfirmed=$true;noAutomaticClaimOrDeliveryConfirmed=$true;syntheticDataConfirmed=$true}) -eq 401)
  Add-Check 'Synthetic encounter finalization rejects an absent staff identity' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/finalize' @{} @{expectedDocumentationVersion=1;expectedDispositionVersion=1;expectedFinalClinicalReviewVersion=1;sourceReviewConfirmed=$true;syntheticOnlyConfirmed=$true}) -eq 401)
  Add-Check 'Synthetic visit closure rejects an absent staff identity' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/close' @{} @{expectedConsultationVersion=1;encounterLockReviewed=$true;syntheticClosureConfirmed=$true}) -eq 401)
  Add-Check 'Professional-claim preparation rejects an absent staff identity' ((Invoke-Status 'GET' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/professional-claim-preparation') -eq 401)
  ...
  Add-Check 'Applicant connection grant rejects an absent applicant access key even when staff-style headers are supplied' (
    (Invoke-Status 'POST' '/api/telehealth/v1/applicants/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/telehealth-request/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb/connection-grants' @{ 'X-AvenChart-Session'='not-an-applicant-session'; 'X-AvenChart-Facility-Id'='10'; 'X-AvenChart-Purpose-Of-Use'='treatment'; 'X-Idempotency-Key'='applicant-connection-auth-proof' } @{expectedVersion=14;browserSupported=$true;cameraAvailable=$true;microphoneAvailable=$true;speakerAvailable=$true;networkQuality='unknown';syntheticDataConfirmed=$true}) -eq 401)
  ...
  Add-Check 'Administrator role cannot sign a physician-owned prescription' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/prescription' $adminVideoHeaders @{expectedDraftVersion=1;noCurrentMedicationsConfirmed=$true;noKnownAllergiesConfirmed=$true;adequateEvaluationConfirmed=$true;syntheticDataConfirmed=$true}) -eq 403)
  Add-Check 'Administrator role cannot read physician final clinical-review evidence' ((Invoke-Status 'GET' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/final-clinical-review' $adminHeaders) -eq 403)
  Add-Check 'Administrator role cannot record physician final clinical-review evidence' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/final-clinical-review' $adminVideoHeaders @{expectedDocumentationVersion=1;expectedDispositionVersion=1;documentationReviewed=$true;physicianResponsibilityConfirmed=$true;noAutomaticClaimOrDeliveryConfirmed=$true;syntheticDataConfirmed=$true}) -eq 403)
  Add-Check 'Administrator role cannot finalize a physician synthetic encounter' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/finalize' $adminVideoHeaders @{expectedDocumentationVersion=1;expectedDispositionVersion=1;expectedFinalClinicalReviewVersion=1;sourceReviewConfirmed=$true;syntheticOnlyConfirmed=$true}) -eq 403)
  Add-Check 'Administrator role cannot close a physician synthetic visit' ((Invoke-Status 'POST' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/close' $adminVideoHeaders @{expectedConsultationVersion=1;encounterLockReviewed=$true;syntheticClosureConfirmed=$true}) -eq 403)
  Add-Check 'Administrator role cannot read physician professional-claim preparation' ((Invoke-Status 'GET' '/api/telehealth/v1/clinician/consultations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/professional-claim-preparation' $adminHeaders) -eq 403)
...
finally{$result=[ordered]@{status=$(if($passed){'passed'}else{'failed'});generatedAtUtc=(Get-Date).ToUniversalTime().ToString('O');decisions=@('TH-DEC-0003','TH-DEC-0005','TH-DEC-0006','TH-DEC-0007','TH-DEC-0008','TH-DEC-0009','TH-DEC-0010','TH-DEC-0011','TH-DEC-0012','TH-DEC-0013','TH-DEC-0014','TH-DEC-0015','TH-DEC-0016','TH-DEC-0017','TH-DEC-0018','TH-DEC-0019','TH-DEC-0020','TH-DEC-0021','TH-DEC-0022','TH-DEC-0023','TH-DEC-0024','TH-DEC-0025','TH-DEC-0026','TH-DEC-0027','TH-DEC-0028','TH-DEC-0029','TH-DEC-0030','TH-DEC-0031','TH-DEC-0032','TH-DEC-0033','TH-DEC-0034','TH-DEC-0035','TH-DEC-0036','TH-DEC-0037','TH-DEC-0038','TH-DEC-0039','TH-DEC-0040','TH-DEC-0041','TH-DEC-0042','TH-DEC-0043','TH-DEC-0044','TH-DEC-0045','TH-DEC-0046','TH-DEC-0047','TH-DEC-0048','TH-DEC-0049','TH-DEC-0050','TH-DEC-0051','TH-DEC-0052','TH-DEC-0053','TH-DEC-0054','TH-DEC-0055','TH-DEC-0056','TH-DEC-0057','TH-DEC-0058','TH-DEC-0059','TH-DEC-0060','TH-DEC-0061','TH-DEC-0062','TH-DEC-0063','TH-DEC-0064');checks=$checks};$result|ConvertTo-Json -Depth 10|Set-Content $resultPath -Encoding utf8;$result|ConvertTo-Json -Depth 10}
