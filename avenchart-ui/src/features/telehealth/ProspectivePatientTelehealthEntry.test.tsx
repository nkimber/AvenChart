...
vi.mock('./api.ts', async (importOriginal) => {
  ...
  return {
    ...
    confirmApplicantTelehealthRequestInsuranceSource: vi.fn(),
    ...
    submitApplicantTelehealthRequestForOperationalReview: vi.fn(),
    ...
    getApplicantTelehealthRequestOperationalReviewSubmission: vi.fn(),
    ...
    runApplicantTelehealthRequestPracticeNetwork: vi.fn(),
    selectApplicantTelehealthRequestRenderingCandidate: vi.fn(),
...
} satisfies TelehealthApplicantRequestIntake

const requestInsuranceSourceFixture = {
  applicantId: approvedApplicant.applicantId,
  applicantVersion: 26,
  applicantStatus: 'SyntheticRequestCreated',
  requestId: requestLocationFixture.requestId,
...
const requestPracticeNetworkFixture = {
  applicantId: approvedApplicant.applicantId,
  applicantVersion: 26,
  applicantStatus: 'SyntheticRequestCreated',
  requestId: requestLocationFixture.requestId,
  ...
  eligibilityBusinessOutcome: 'EligibleBenefitsReported',
  eligibilityCheckedAt: '2026-08-28T17:13:00Z',
  eligibilityExpiresAt: '2026-08-28T17:28:00Z',
  verificationReady: true,
  verificationCompleted: false,
  verificationId: null,
  dateOfService: null,
  ...
  integrationEnabled: false,
  externalCallPerformed: false,
  direction: 'Run one fresh synthetic practice-network check.',
  limitations: ['No external directory or payer will be contacted.'],
} satisfies TelehealthApplicantRequestPracticeNetwork

const requestRenderingCandidateFixture = {
  applicantId: approvedApplicant.applicantId,
  applicantVersion: 26,
  applicantStatus: 'SyntheticRequestCreated',
  requestId: requestLocationFixture.requestId,
  ...
  practiceNetworkBusinessOutcome: 'PracticeInNetworkAcceptingNewPatients',
  practiceNetworkCheckedAt: '2026-08-28T17:14:00Z',
  practiceNetworkExpiresAt: '2026-08-28T17:29:00Z',
...
} satisfies TelehealthApplicantRequestRenderingCandidate

const requestParticipationContextFixture = {
  applicantId: approvedApplicant.applicantId,
  applicantVersion: 26,
  applicantStatus: 'SyntheticRequestCreated',
  requestId: requestLocationFixture.requestId,
  ...
  confirmationId: null,
  confirmedAt: null,
  participationEvaluationContextConfirmed: false,
  ...
  integrationEnabled: false,
  externalCallPerformed: false,
  direction: 'Review the server-owned synthetic prerequisite context.',
...
const requestOperationalReviewSubmissionFixture = {
  applicantId: approvedApplicant.applicantId,
  applicantVersion: 26,
  applicantStatus: 'SyntheticRequestCreated',
  requestId: requestLocationFixture.requestId,
...
} satisfies TelehealthApplicantRequestOperationalReviewSubmission
...
describe('ProspectivePatientTelehealthEntry safety triage', () => {
  beforeEach(() => {
    ...
    vi.mocked(getApplicantTelehealthRequestOperationalReviewSubmission).mockResolvedValue(requestOperationalReviewSubmissionFixture)
  ...
  it('creates one authorized Draft request with stable retry and no queue or care authority', async () => {
    vi.spyOn(globalThis.crypto, 'randomUUID').mockReturnValue('1d000000-0000-4000-8000-00000000001d')
    ...
    vi.mocked(getProspectiveApplicant).mockResolvedValue(authorizedApplicant)
    vi.mocked(getApplicantTelehealthRequest)
      .mockRejectedValueOnce(new Error('Request state temporarily unavailable.'))
      .mockResolvedValueOnce(ready)
      .mockResolvedValue(created)
    vi.mocked(createApplicantTelehealthRequest)
      .mockRejectedValueOnce(new Error('Creation result unknown; retry unchanged.'))
      .mockResolvedValue(created)

    render(<MemoryRouter><ProspectivePatientTelehealthEntry /></MemoryRouter>)

    const loadFailure = await screen.findByRole('alert')
    expect(loadFailure).toHaveTextContent('Request state temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Retry request-creation load' }))

    const heading = await screen.findByRole('heading', { name: 'Create the authorized Draft request' })
    expect(heading.parentElement).toHaveTextContent('Doctor search or queueNot started')
    expect(heading.parentElement?.querySelector('input[type="text"]')).toBeNull()
    expect(heading.parentElement?.querySelector('textarea')).toBeNull()
    fireEvent.click(screen.getByLabelText(/create this one synthetic Draft/i))
    fireEvent.click(screen.getByLabelText(/does not start a doctor search/i))
    fireEvent.click(screen.getByLabelText(/urgent or worsening/i))
    fireEvent.click(screen.getByRole('button', { name: 'Create Draft telehealth request' }))

    const creationFailure = await screen.findByRole('alert')
    expect(creationFailure).toHaveTextContent('Creation result unknown; retry unchanged.')
    expect(screen.getByLabelText(/create this one synthetic Draft/i)).toBeChecked()
    fireEvent.click(screen.getByRole('button', { name: 'Create Draft telehealth request' }))

    await waitFor(() => expect(createApplicantTelehealthRequest).toHaveBeenCalledTimes(2))
    expect(vi.mocked(createApplicantTelehealthRequest).mock.calls[0][3]).toBe('1d000000-0000-4000-8000-00000000001d')
    expect(vi.mocked(createApplicantTelehealthRequest).mock.calls[1][3]).toBe('1d000000-0000-4000-8000-00000000001d')
    expect(vi.mocked(createApplicantTelehealthRequest).mock.calls[0][2]).toEqual({
    ...
    const result = await screen.findByRole('heading', { name: 'Draft telehealth request created' })
    expect(result.parentElement).toHaveTextContent('Request statusDraft')
    expect(result.parentElement).toHaveTextContent('Doctor search startedNo')
    expect(result.parentElement).toHaveTextContent('Patient or clinician queue enteredNo')
    expect(result.parentElement).toHaveTextContent('Queue position assignedNo')
    await waitFor(() => expect(result.parentElement).toHaveFocus())
  ...
  it('recovers and confirms a no-free-text request intake with one duration and eight explicit acknowledgments', async () => {
    const requestCreatedApplicant = {
      ...approvedApplicant,
      status: 'SyntheticRequestCreated',
      version: 26,
      canonicalPatientCreated: true,
      nextAction: 'Confirm the request intake snapshot.',
    } as const
    const requestCreationReceipt = {
      applicantId: approvedApplicant.applicantId,
      applicantVersion: 26,
    ...
    vi.mocked(getApplicantTelehealthRequestLocation).mockResolvedValue(confirmedLocation)
    vi.mocked(getApplicantTelehealthRequestUniversalSafety).mockResolvedValue(universalPass)
    vi.mocked(getApplicantTelehealthRequestComplaintTriage).mockResolvedValue(complaintPass)
    vi.mocked(getApplicantTelehealthRequestIntake)
      .mockRejectedValueOnce(new Error('Intake projection temporarily unavailable.'))
      .mockResolvedValue(requestIntakeFixture)
    vi.mocked(confirmApplicantTelehealthRequestIntake)
      .mockRejectedValueOnce(new Error('Intake result unknown; retry unchanged.'))
      .mockResolvedValue(completedIntake)

    render(<MemoryRouter><ProspectivePatientTelehealthEntry /></MemoryRouter>)

    expect(await screen.findByRole('alert')).toHaveTextContent('Intake projection temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Retry request intake load' }))
    const heading = await screen.findByRole('heading', { name: 'Confirm request intake snapshot' })
    const form = heading.closest('form')
    expect(form).not.toBeNull()
    expect(form?.querySelector('textarea')).toBeNull()
    expect(form?.querySelector('input[type="text"]')).toBeNull()
    expect(form).toHaveTextContent('Server-owned purposeSleep difficulty')
    expect(form).toHaveTextContent('UNAPPROVED_SYNTHETIC')
    expect(form).toHaveTextContent('Canonical coverage pending')

    fireEvent.change(within(form!).getByLabelText('How long has the fictional concern been present?'), {
    ...
    fireEvent.click(within(form!).getByRole('button', { name: 'Record synthetic intake snapshot' }))

    expect(await screen.findByRole('alert')).toHaveTextContent('Intake result unknown; retry unchanged.')
    expect(within(form!).getByLabelText('How long has the fictional concern been present?')).toHaveValue('1-3-days')
    fireEvent.click(within(form!).getByRole('button', { name: 'Record synthetic intake snapshot' }))

    await waitFor(() => expect(confirmApplicantTelehealthRequestIntake).toHaveBeenCalledTimes(2))
    expect(vi.mocked(confirmApplicantTelehealthRequestIntake).mock.calls[0][3]).toBe(
      vi.mocked(confirmApplicantTelehealthRequestIntake).mock.calls[1][3],
    )
    expect(vi.mocked(confirmApplicantTelehealthRequestIntake).mock.calls[0][2]).toEqual({
    ...
    const result = await screen.findByRole('heading', { name: 'Request intake snapshot recorded' })
    expect(result.parentElement).toHaveTextContent('Request statusVerification')
    expect(result.parentElement).toHaveTextContent('Verification pendingYes')
    expect(result.parentElement).toHaveTextContent('Consent pendingYes')
    expect(result.parentElement).toHaveTextContent('Coverage or exact network confirmedNo')
    expect(result.parentElement).toHaveTextContent('Operational review or practice acceptanceNo')
    expect(result.parentElement).toHaveTextContent('Doctor search or queueNot started')
    await waitFor(() => expect(result.parentElement).toHaveFocus())
  ...
  it('recovers the masked source and fresh request eligibility with stable retry identities', async () => {
    const requestCreatedApplicant = {
      ...
      version: 26,
      canonicalPatientCreated: true,
      nextAction: 'Confirm the request insurance source.',
    } as const
    const requestCreationReceipt = {
      applicantId: approvedApplicant.applicantId,
      applicantVersion: 26,
    ...
    const completedPracticeNetwork = {
      ...requestPracticeNetworkFixture,
      requestVersion: 8,
      verificationReady: false,
      verificationCompleted: true,
    ...
    const completedRenderingCandidate = {
      ...requestRenderingCandidateFixture,
      requestVersion: 9,
      ...
      direction: 'This synthetic clinician is bound only as the candidate for a future exact participation check.',
    } satisfies TelehealthApplicantRequestRenderingCandidate
    const completedParticipationContext = {
      ...requestParticipationContextFixture,
      requestVersion: 10,
      confirmationReady: false,
      confirmationCompleted: true,
      confirmationId: '49000000-0000-4000-8000-000000000001',
      confirmedAt: '2026-08-28T17:16:00Z',
    ...
    const completedOperationalReviewSubmission = {
      ...requestOperationalReviewSubmissionFixture,
      requestVersion: 12,
      requestStatus: 'OperationalReview',
      submissionReady: false,
      submissionCompleted: true,
      submittedAt: '2026-08-29T17:18:00Z',
    ...
    vi.mocked(getApplicantTelehealthRequestUniversalSafety).mockResolvedValue(universalPass)
    vi.mocked(getApplicantTelehealthRequestComplaintTriage).mockResolvedValue(complaintPass)
    vi.mocked(getApplicantTelehealthRequestIntake).mockResolvedValue(completedIntake)
    vi.mocked(getApplicantTelehealthRequestInsuranceSource)
      .mockRejectedValueOnce(new Error('Insurance-source projection temporarily unavailable.'))
      .mockResolvedValue(requestInsuranceSourceFixture)
    ...
    vi.mocked(getApplicantTelehealthRequestPracticeNetwork)
      .mockRejectedValueOnce(new Error('Practice-network projection temporarily unavailable.'))
      .mockResolvedValue(requestPracticeNetworkFixture)
    vi.mocked(runApplicantTelehealthRequestPracticeNetwork)
      .mockRejectedValueOnce(new Error('Practice-network result unknown; retry unchanged.'))
      .mockResolvedValue(completedPracticeNetwork)
    vi.mocked(getApplicantTelehealthRequestRenderingCandidate)
      .mockRejectedValueOnce(new Error('Rendering-candidate projection temporarily unavailable.'))
      .mockResolvedValue(requestRenderingCandidateFixture)
    vi.mocked(selectApplicantTelehealthRequestRenderingCandidate)
      .mockRejectedValueOnce(new Error('Rendering-candidate result unknown; retry unchanged.'))
      .mockResolvedValue(completedRenderingCandidate)
    vi.mocked(getApplicantTelehealthRequestParticipationContext)
      .mockRejectedValueOnce(new Error('Participation-context projection temporarily unavailable.'))
      .mockResolvedValue(requestParticipationContextFixture)
    ...
    vi.mocked(getApplicantTelehealthRequestOperationalReviewSubmission)
      .mockRejectedValueOnce(new Error('Operational-review submission temporarily unavailable.'))
      .mockResolvedValue(requestOperationalReviewSubmissionFixture)
    vi.mocked(submitApplicantTelehealthRequestForOperationalReview)
      .mockRejectedValueOnce(new Error('Operational-review submission result unknown; retry unchanged.'))
      .mockResolvedValue(completedOperationalReviewSubmission)

    render(<MemoryRouter><ProspectivePatientTelehealthEntry /></MemoryRouter>)

    expect(await screen.findByRole('alert')).toHaveTextContent('Insurance-source projection temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Retry insurance-source load' }))
    const heading = await screen.findByRole('heading', { name: 'Confirm insurance source for this request' })
    const form = heading.closest('form')
    expect(form).not.toBeNull()
    expect(form?.querySelector('textarea')).toBeNull()
    expect(form?.querySelector('select')).toBeNull()
    expect(form?.querySelector('input:not([type="checkbox"])')).toBeNull()
    expect(form).toHaveTextContent('AvenChart Synthetic Health')
    expect(form).toHaveTextContent('••••4123')
    expect(form).toHaveTextContent('Earlier eligibility result — historical only')
    expect(form).toHaveTextContent('Earlier result reusableNo')
    expect(form).toHaveTextContent('Earlier treating-physician network checkNot performed')
    ...
    fireEvent.click(within(form!).getByRole('button', { name: 'Confirm synthetic insurance source' }))

    expect(await screen.findByRole('alert')).toHaveTextContent('Insurance-source result unknown; retry unchanged.')
    fireEvent.click(within(form!).getByRole('button', { name: 'Confirm synthetic insurance source' }))

    await waitFor(() => expect(confirmApplicantTelehealthRequestInsuranceSource).toHaveBeenCalledTimes(2))
    expect(vi.mocked(confirmApplicantTelehealthRequestInsuranceSource).mock.calls[0][3]).toBe(
      vi.mocked(confirmApplicantTelehealthRequestInsuranceSource).mock.calls[1][3],
    )
    expect(vi.mocked(confirmApplicantTelehealthRequestInsuranceSource).mock.calls[0][2]).toEqual({
    ...
    const result = await screen.findByRole('heading', { name: 'Request insurance source confirmed' })
    expect(result.parentElement).toHaveTextContent('Request statusVerification')
    expect(result.parentElement).toHaveTextContent('Request version6')
    expect(result.parentElement).toHaveTextContent('Intent recorded; not performed')
    expect(result.parentElement).toHaveTextContent('Current coverage verifiedNo')
    expect(result.parentElement).toHaveTextContent('Exact network confirmedNo')
    expect(result.parentElement).toHaveTextContent('Doctor search or queueNot started')
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Practice-network projection temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Retry practice-network load' }))
    const networkHeading = await screen.findByRole('heading', { name: 'Verify the practice-level network fixture' })
    const networkForm = networkHeading.closest('form')
    expect(networkForm).not.toBeNull()
    expect(networkForm?.querySelector('textarea')).toBeNull()
    expect(networkForm?.querySelector('select')).toBeNull()
    expect(networkForm?.querySelector('input:not([type="checkbox"])')).toBeNull()
    expect(networkForm).toHaveTextContent('AvenChart Synthetic Practice')
    expect(networkForm).toHaveTextContent('EligibleBenefitsReported')
    expect(networkForm).toHaveTextContent('cannot establish exact network status because no rendering physician has been selected')
    for (const label of [
      ...
      fireEvent.click(within(networkForm!).getByLabelText(label))
    }
    fireEvent.click(within(networkForm!).getByRole('button', { name: 'Run synthetic practice-network check' }))

    expect(await screen.findByRole('alert')).toHaveTextContent('Practice-network result unknown; retry unchanged.')
    fireEvent.click(within(networkForm!).getByRole('button', { name: 'Run synthetic practice-network check' }))
    await waitFor(() => expect(runApplicantTelehealthRequestPracticeNetwork).toHaveBeenCalledTimes(2))
    expect(vi.mocked(runApplicantTelehealthRequestPracticeNetwork).mock.calls[0][3]).toBe(
      vi.mocked(runApplicantTelehealthRequestPracticeNetwork).mock.calls[1][3],
    )
    expect(vi.mocked(runApplicantTelehealthRequestPracticeNetwork).mock.calls[0][2]).toEqual({
    ...
    const networkResult = await screen.findByRole('heading', { name: 'Fresh practice-network result recorded' })
    expect(networkResult.parentElement).toHaveTextContent('Request version8')
    expect(networkResult.parentElement).toHaveTextContent('Practice affiliationInNetwork')
    expect(networkResult.parentElement).toHaveTextContent('Accepting new patientsAccepting')
    expect(networkResult.parentElement).toHaveTextContent('Rendering physician selectedNo')
    expect(networkResult.parentElement).toHaveTextContent('Exact network confirmedNo — physician check still required')
    expect(networkResult.parentElement).toHaveTextContent('Doctor search or queueNot started')
    await waitFor(() => expect(networkResult.parentElement).toHaveFocus())

    expect(await screen.findByRole('alert')).toHaveTextContent('Rendering-candidate projection temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Retry rendering-candidate load' }))
    const candidateHeading = await screen.findByRole('heading', { name: 'Review a candidate for network evaluation' })
    const candidateForm = candidateHeading.closest('form')
    expect(candidateForm).not.toBeNull()
    expect(candidateForm?.querySelector('textarea')).toBeNull()
    expect(candidateForm?.querySelector('select')).toBeNull()
    expect(candidateForm?.querySelector('input:not([type="checkbox"])')).toBeNull()
    expect(candidateForm).toHaveTextContent('Georgia Synthetic Clinician')
    expect(candidateForm).toHaveTextContent('Synthetic provider ••••8101')
    expect(candidateForm).not.toHaveTextContent('18888101')
    expect(candidateForm).toHaveTextContent('does not assign the clinician or establish network status')
    for (const label of [
      ...
      fireEvent.click(within(candidateForm!).getByLabelText(label))
    }
    fireEvent.click(within(candidateForm!).getByRole('button', { name: 'Select candidate for network evaluation' }))

    expect(await screen.findByRole('alert')).toHaveTextContent('Rendering-candidate result unknown; retry unchanged.')
    fireEvent.click(within(candidateForm!).getByRole('button', { name: 'Select candidate for network evaluation' }))
    await waitFor(() => expect(selectApplicantTelehealthRequestRenderingCandidate).toHaveBeenCalledTimes(2))
    expect(vi.mocked(selectApplicantTelehealthRequestRenderingCandidate).mock.calls[0][3]).toBe(
      vi.mocked(selectApplicantTelehealthRequestRenderingCandidate).mock.calls[1][3],
    )
    expect(vi.mocked(selectApplicantTelehealthRequestRenderingCandidate).mock.calls[0][2]).toEqual({
    ...
    const candidateResult = await screen.findByRole('heading', { name: 'Rendering candidate selected for network evaluation' })
    expect(candidateResult.parentElement).toHaveTextContent('Request version9')
    expect(candidateResult.parentElement).toHaveTextContent('Candidate selected for network evaluationYes')
    expect(candidateResult.parentElement).toHaveTextContent('Clinician assignedNo')
    expect(candidateResult.parentElement).toHaveTextContent('Physician network checkedNo — still required')
    expect(candidateResult.parentElement).toHaveTextContent('Exact network confirmedNo')
    expect(candidateResult.parentElement).toHaveTextContent('Doctor search or queueNot started')
    await waitFor(() => expect(candidateResult.parentElement).toHaveFocus())

    expect(await screen.findByRole('alert')).toHaveTextContent('Participation-context projection temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Retry participation-context load' }))
    const contextHeading = await screen.findByRole('heading', { name: 'Review participation-evaluation prerequisites' })
    const contextForm = contextHeading.closest('form')
    expect(contextForm).not.toBeNull()
    expect(contextForm?.querySelector('textarea')).toBeNull()
    expect(contextForm?.querySelector('select')).toBeNull()
    expect(contextForm?.querySelector('input:not([type="checkbox"])')).toBeNull()
    expect(contextForm).toHaveTextContent('Georgia Synthetic Clinician')
    expect(contextForm).toHaveTextContent('Synthetic provider ••••8101')
    expect(contextForm).toHaveTextContent('Synthetic billing provider ••••8800')
    expect(contextForm).not.toHaveTextContent('18888101')
    expect(contextForm).not.toHaveTextContent('syn-contract')
    expect(contextForm).toHaveTextContent('Fixture presence is not real verification')
    for (const label of [
      'I confirm this is a fictional synthetic demonstration context.',
      'I understand an NPI is an identifier and does not prove licensure, credentials, participation, or availability.',
      'I understand no real state authority or credentialing source has been verified.',
      'I understand exact billing-entity and rendering-provider participation is still unchecked.',
    ]) {
      fireEvent.click(within(contextForm!).getByLabelText(label))
    }
    fireEvent.click(within(contextForm!).getByRole('button', { name: 'Confirm participation prerequisite context' }))

    expect(await screen.findByRole('alert')).toHaveTextContent('Participation-context result unknown; retry unchanged.')
    fireEvent.click(within(contextForm!).getByRole('button', { name: 'Confirm participation prerequisite context' }))
    await waitFor(() => expect(confirmApplicantTelehealthRequestParticipationContext).toHaveBeenCalledTimes(2))
    expect(vi.mocked(confirmApplicantTelehealthRequestParticipationContext).mock.calls[0][3]).toBe(
      vi.mocked(confirmApplicantTelehealthRequestParticipationContext).mock.calls[1][3],
    )
    expect(vi.mocked(confirmApplicantTelehealthRequestParticipationContext).mock.calls[0][2]).toEqual({
    ...
    const contextResult = await screen.findByRole('heading', { name: 'Participation prerequisite context confirmed' })
    expect(contextResult.parentElement).toHaveTextContent('Request version10')
    expect(contextResult.parentElement).toHaveTextContent('Evaluation context confirmedYes — prerequisites only')
    expect(contextResult.parentElement).toHaveTextContent('Real state authority verifiedNo — still required')
    expect(contextResult.parentElement).toHaveTextContent('Real credentialing verifiedNo — still required')
    expect(contextResult.parentElement).toHaveTextContent('Physician network checkedNo — still required')
    expect(contextResult.parentElement).toHaveTextContent('Exact network confirmedNo')
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Operational-review submission temporarily unavailable.')
    fireEvent.click(screen.getByRole('button', { name: 'Try loading submission again' }))
    const submissionGroup = await screen.findByRole('group', { name: 'Submit the synthetic request for practice review' })
    const submissionForm = submissionGroup.closest('form')
    expect(submissionForm).not.toBeNull()
    expect(submissionForm?.querySelector('textarea')).toBeNull()
    expect(submissionForm?.querySelector('select')).toBeNull()
    expect(submissionForm?.querySelector('input:not([type="checkbox"])')).toBeNull()
    expect(submissionForm).toHaveTextContent('AvenChart Synthetic Practice')
    expect(submissionForm).toHaveTextContent('Synthetic provider ••••8101')
    expect(submissionForm).toHaveTextContent('Synthetic billing provider ••••8800')
    expect(submissionForm).not.toHaveTextContent('18888101')
    expect(submissionForm).not.toHaveTextContent('syn-contract')
    for (const label of [
      ...
      fireEvent.click(within(submissionForm!).getByLabelText(label))
    }
    fireEvent.click(within(submissionForm!).getByRole('button', { name: 'Submit for practice review' }))

    expect(await screen.findByRole('alert')).toHaveTextContent('Operational-review submission result unknown; retry unchanged.')
    fireEvent.click(within(submissionForm!).getByRole('button', { name: 'Submit for practice review' }))
    await waitFor(() => expect(submitApplicantTelehealthRequestForOperationalReview).toHaveBeenCalledTimes(2))
    expect(vi.mocked(submitApplicantTelehealthRequestForOperationalReview).mock.calls[0][3]).toBe(
      vi.mocked(submitApplicantTelehealthRequestForOperationalReview).mock.calls[1][3],
    )
    expect(vi.mocked(submitApplicantTelehealthRequestForOperationalReview).mock.calls[0][2]).toEqual({
    ...
    const submissionResult = await screen.findByRole('heading', { name: 'Submitted for practice review' })
    expect(submissionResult.parentElement).toHaveTextContent('Request statusOperationalReview')
    expect(submissionResult.parentElement).toHaveTextContent('Request version12')
    expect(submissionResult.parentElement).toHaveTextContent('Synthetic checks complete')
    expect(submissionResult.parentElement).toHaveTextContent('Awaiting staff review')
    expect(submissionResult.parentElement).toHaveTextContent('Practice acceptedNo — review pending')
    expect(submissionResult.parentElement).toHaveTextContent('Coverage verifiedNo — still required')
    expect(submissionResult.parentElement).toHaveTextContent('Doctor search or queueNot started')
    await waitFor(() => expect(submissionResult.parentElement).toHaveFocus())

    const stored = `${sessionStorage.getItem('avenchart-ui.telehealthProspectiveApplicant') ?? ''}${localStorage.getItem('avenchart-ui.telehealthProspectiveApplicant') ?? ''}`
    expect(stored).not.toMatch(/insuranceSourceSnapshotFingerprint|eligibilitySnapshotFingerprint|networkSnapshotFingerprint|candidateSnapshotFingerprint|contextSnapshotFingerprint|evaluationSnapshotFingerprint|submissionSnapshotFingerprint|payerDisplayName|maskedMemberId|eligibilityBusinessOutcome|networkBusinessOutcome|businessOutcome|candidateDisplayName|providerReference|billingProviderReference|contractReference/i)
  }, 30_000)
})
