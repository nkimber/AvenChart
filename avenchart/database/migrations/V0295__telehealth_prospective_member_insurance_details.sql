...
-- TH-DEC-0021: one protected synthetic prospective member-insurance receipt.
-- This is not member matching, eligibility, exact network, coverage, or care.
...
alter table telehealth_prospective_applicants
  add constraint chk_telehealth_applicant_status check (
    status in ('ContactVerificationPending','IdentityReviewPending',
               ...
               'MemberInsuranceDetailsRecorded',
...
alter table telehealth_applicant_events
  add constraint chk_telehealth_applicant_event_status check (
    (from_status is null or from_status in (
      ...
      'SafetyInPersonRequired','SafetyEmergencyRedirect',
      'VisitPurposeRecorded','PracticeNetworkPrecheckRecorded',
      'MemberInsuranceDetailsRecorded','VerificationLocked','Expired'))
    and to_status in (
      'ContactVerificationPending','IdentityReviewPending',
      'IdentityReviewApproved','ManualReviewRequired',
      'SafetyScreenPassed','SafetyClinicalReviewRequired',
...
create table if not exists telehealth_applicant_member_insurance_details (
  ...
  constraint uq_telehealth_applicant_member_details_idempotency
    unique (applicant_id,idempotency_key),
  constraint chk_telehealth_applicant_member_details_practice
    check (practice_id ~ '^[a-z0-9][a-z0-9-]{1,78}[a-z0-9]$'),
  constraint chk_telehealth_applicant_member_details_version
    check (resulting_applicant_version >= 7),
  constraint chk_telehealth_applicant_member_details_status
    check (resulting_applicant_status='MemberInsuranceDetailsRecorded'),
  constraint chk_telehealth_applicant_member_details_location
    check (location_state_code in ('GA','CA','FL')),
  constraint chk_telehealth_applicant_member_details_purpose
    check (purpose_category in ('migraine','sleep')),
  constraint chk_telehealth_applicant_member_details_plan_status check (
  ...
  constraint chk_telehealth_applicant_member_details_relationship
    check (subscriber_relationship in ('Self','Spouse','Parent','Other')),
  constraint chk_telehealth_applicant_member_details_priority
    check (coverage_priority='Primary'),
  constraint chk_telehealth_applicant_member_details_masks check (
  ...
  constraint chk_telehealth_applicant_member_details_confirmations
    check (details_confirmed and synthetic_data_confirmed),
  constraint chk_telehealth_applicant_member_details_protection check (
    length(protected_payload) between 64 and 8192
    and protection_scheme='ASP.NET_CORE_DATA_PROTECTION'
    and protection_purpose='AvenChart.Telehealth.ProspectiveMemberInsuranceDetails.v1'
    and protection_version=1),
  constraint chk_telehealth_applicant_member_details_idempotency
    check (length(idempotency_key) between 8 and 128),
  constraint chk_telehealth_applicant_member_details_fingerprint
    check (command_fingerprint ~ '^[0-9a-f]{64}$'),
  constraint chk_telehealth_applicant_member_details_no_consequence check (
    not member_matched and not member_eligibility_checked
...
create or replace function enforce_telehealth_applicant_member_insurance_details()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  select * into precheck_row
  from telehealth_applicant_practice_network_prechecks
  where precheck_id=new.practice_network_precheck_id
    and applicant_id=new.applicant_id;
  ...
  if applicant_row.applicant_id is null
     ...
     or precheck_row.location_state_code <> new.location_state_code
     or precheck_row.purpose_category <> new.purpose_category
     or precheck_row.plan_key <> new.plan_key
     or precheck_row.payer_display_name <> new.payer_display_name
     or precheck_row.product_display_name <> new.product_display_name
     or precheck_row.practice_network_status <> new.practice_network_status
    ...
    raise exception using
      errcode='P0001',
      message='telehealth_applicant_member_insurance_details_snapshot_mismatch';
  end if;
...
drop trigger if exists trg_telehealth_applicant_member_details_guard
  on telehealth_applicant_member_insurance_details;
create trigger trg_telehealth_applicant_member_details_guard
before insert on telehealth_applicant_member_insurance_details
for each row execute function enforce_telehealth_applicant_member_insurance_details();

drop trigger if exists trg_telehealth_applicant_member_details_append_only
  on telehealth_applicant_member_insurance_details;
create trigger trg_telehealth_applicant_member_details_append_only
before update or delete on telehealth_applicant_member_insurance_details
for each row execute function reject_telehealth_evidence_mutation();

create index if not exists ix_telehealth_applicant_member_details_recorded
  on telehealth_applicant_member_insurance_details(
