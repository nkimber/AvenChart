...
try {
    ...
    Add-Check 'Applicant queue-status response is minimized, approximate-only, and explicit about false care consequences' (
      @(Compare-Object @('approximateRequestsAhead','careAuthorized','consentCreated','coverageVerified','detail','doctorSearchStarted','exactQueuePositionAssigned','externalCallPerformed','headline','integrationEnabled','limitations','phase','policyKey','policyVersion','positionIsApproximate','practiceAccepted','realtimeAvailable','refreshAfterSeconds','renderingPhysicianAssigned','renderingPhysicianIdentityDisclosed','requestId','requestStatus','requestUpdatedAt','requestVersion','safetyActions','snapshotAt','sourceMode','waitEstimateAvailable','waitEstimateMessage') ($applicantQueueStatusResponseProperties|Sort-Object)).Count -eq 0 -and
