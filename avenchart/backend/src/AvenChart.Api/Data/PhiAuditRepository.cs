...
public sealed class PhiAuditRepository(NpgsqlDataSource dataSource)
{
    public async Task RecordAccessDecisionAsync(
        ...
        PhiAuditResource? resource = null)
    {
        ...
        command.CommandText = """
            insert into phi_access_audit_events (
              ...
              required_permission, resource_type, resource_id, facility_id, facility_code, purpose_of_use, authorized, response_status
            ) values (
              @audit_id, @occurred_at, @username, @session_id, @http_method, @endpoint_name,
              @required_permission, @resource_type, @resource_id, @facility_id, @facility_code, @purpose_of_use, @authorized, @response_status
            );
        ...
        command.Parameters.Add("resource_type", NpgsqlDbType.Text).Value = (object?)resource?.Type ?? DBNull.Value;
        command.Parameters.Add("resource_id", NpgsqlDbType.Text).Value = (object?)resource?.Id ?? DBNull.Value;
    ...
    public async Task<PhiAccessAuditResponse> GetRecentAsync(int limit, string? username, DateOnly? from, DateOnly? to, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            select audit_id, occurred_at, username, http_method, endpoint_name, required_permission,
              resource_type, resource_id, facility_id, facility_code, purpose_of_use, authorized, response_status
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            events.Add(new PhiAccessAuditEventItem(
                ...
                reader.GetBoolean(11), reader.GetInt32(12)));
        }
    ...
    public async Task<string> GetCsvAsync(int limit, string? username, DateOnly? from, DateOnly? to, CancellationToken cancellationToken)
    {
        ...
        var csv = new System.Text.StringBuilder("Occurred At,Username,Method,Endpoint,Required Permission,Resource Type,Resource ID,Facility ID,Facility Code,Purpose Of Use,Decision,Response Status\n");
        foreach (var entry in response.Events)
        {
            csv.AppendLine(string.Join(',', new[]
            {
                ...
                EscapeCsv(entry.EndpointName), EscapeCsv(entry.RequiredPermission),
                EscapeCsv(entry.ResourceType ?? string.Empty), EscapeCsv(entry.ResourceId ?? string.Empty), EscapeCsv(entry.FacilityId?.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? string.Empty),
