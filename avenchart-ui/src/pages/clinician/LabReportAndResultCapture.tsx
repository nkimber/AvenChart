...
export default function LabReportAndResultCapture({ sessionId, orders, onChange }: Props) {
  const [report, setReport] = useState({ orderId: '', specimenNumber: '', dateCollected: today(), dateReport: today(), notes: '' })
  ...
  async function submitReport(event: FormEvent) {
    ...
    setBusy(true)
    try {
      onChange(await createProcedureReport(sessionId, { orderId, specimenNumber: report.specimenNumber.trim(), dateCollected: `${report.dateCollected}T12:00:00`, dateReport: `${report.dateReport}T12:00:00`, reportStatus: 'received', reviewStatus: 'received', notes: report.notes.trim() }))
