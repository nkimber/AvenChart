...
type ThreadState = { status: 'idle' } | { status: 'loading'; patient: ThreadPatient } | { status: 'ready'; thread: PatientThread } | { status: 'error'; patient: ThreadPatient; message: string }
...
export default function ClinicianMessages() {
  ...
  const [escalationHistoryLoadingId, setEscalationHistoryLoadingId] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    setInboxError(null)
    getStaffMessageInbox(session.sessionId, queryFromParams(new URLSearchParams(queryKey)), controller.signal)
      .then((result) => {
        setInbox(result)
        setLastUpdated(
  ...
  async function uploadAttachment(messageId: string) {
    ...
    if (!allowed.has(file.type) || file.size === 0 || file.size > 4 * 1024 * 1024) {
      showToast('Choose a PDF, PNG, JPEG, or text file from 1 byte to 4 MiB.', 'error')
      return
    }
    ...
    try {
      ...
      await uploadStaffMessageAttachment(session.sessionId, messageId, {
        fileName: file.name,
      ...
      showToast(error instanceof Error ? error.message : 'Could not add attachment.', 'error')
    } finally {
  ...
  async function downloadAttachment(messageId: string, attachment: StaffMessageAttachmentItem) {
    try {
      const blob = await downloadStaffMessageAttachment(session.sessionId, messageId, attachment.id)
      ...
      showToast(error instanceof Error ? error.message : 'Could not download attachment.', 'error')
    }
  }
  ...
  function updateCorrectionDraft(messageId: string, field: 'correction' | 'reason', value: string) {
    setCorrectionDrafts((current) => ({
      ...current,
      [messageId]: {
        correction: current[messageId]?.correction ?? '',
  ...
  async function handleCorrection(message: PatientMessageItem) {
    ...
    try {
      const updated = await correctStaffMessage(session.sessionId, message.id, {
      ...
      setThreadState((previous) =>
        previous.status === 'ready'
          ? {
              ...
              thread: {
                ...previous.thread,
                messages: updated.messages.filter((item) => !item.deleted),
              },
            }
          : previous,
      )
  ...
  async function toggleCorrectionHistory(messageId: string) {
    if (correctionHistory[messageId]) {
      setCorrectionHistory((current) => {
        const next = { ...current }
    ...
    try {
      setCorrectionHistory((current) => current)
      const history = await getStaffMessageCorrectionHistory(session.sessionId, messageId)
      setCorrectionHistory((current) => ({ ...current, [messageId]: history }))
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Could not load correction history.', 'error')
    }
  }
  ...
  async function archiveMessage(message: PatientMessageItem) {
    ...
    try {
      await archiveStaffMessage(session.sessionId, message.id, reason)
      if (threadState.status === 'ready') {
        openThread(threadState.thread.patient, includeArchived)
      }
      setArchiveOpenId(null)
      setArchiveReason((current) => {
        const next = { ...current }
        delete next[message.id]
        return next
      })
      setReload((value) => value + 1)
      showToast('Message archived. Use Show archived to review or restore it.', 'success')
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Could not archive the message.', 'error')
    } finally {
  ...
  async function restoreMessage(message: PatientMessageItem) {
    ...
    setArchiveBusyId(message.id)
    try {
      await restoreStaffMessage(session.sessionId, message.id, reason)
      if (threadState.status === 'ready') {
        openThread(threadState.thread.patient, includeArchived)
      }
      setArchiveOpenId(null)
      ...
      showToast(error instanceof Error ? error.message : 'Could not restore the message.', 'error')
    } finally {
      setArchiveBusyId(null)
    }
  }
  ...
  async function toggleRetentionHistory(messageId: string) {
    if (retentionHistory[messageId]) {
      setRetentionHistory((current) => {
        const next = { ...current }
        delete next[messageId]
        return next
      })
    ...
    try {
      const history = await getStaffMessageRetentionHistory(session.sessionId, messageId)
      setRetentionHistory((current) => ({ ...current, [messageId]: history }))
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Could not load archive history.', 'error')
    }
  }
  ...
  async function handleEscalation(message: PatientMessageItem, escalated: boolean) {
    ...
    try {
      const history = await setStaffMessageEscalation(session.sessionId, message.id, escalated, reason)
      setEscalationHistory((current) => ({
        ...current,
        [message.id]: history,
      }))
      setEscalationOpenId(null)
      setEscalationReason((current) => ({ ...current, [message.id]: '' }))
      showToast(escalated ? 'Message escalation recorded.' : 'Message escalation resolved.', 'success')
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Could not record escalation.', 'error')
    } finally {
  ...
  async function toggleEscalationHistory(messageId: string) {
    setEscalationHistoryLoadingId(messageId)
    try {
      const history = await getStaffMessageEscalationHistory(session.sessionId, messageId)
      setEscalationHistory((current) => ({ ...current, [messageId]: history }))
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Could not load escalation history.', 'error')
    } finally {
      setEscalationHistoryLoadingId(null)
    }
  }

  async function toggleEscalationPanel(messageId: string) {
    if (escalationOpenId === messageId) {
      setEscalationOpenId(null)
      return
    }
    if (!escalationHistory[messageId]) {
      setEscalationHistoryLoadingId(messageId)
      try {
        const history = await getStaffMessageEscalationHistory(session.sessionId, messageId)
        setEscalationHistory((current) => ({
        ...
        showToast(error instanceof Error ? error.message : 'Could not load escalation state.', 'error')
      } finally {
        setEscalationHistoryLoadingId(null)
      }
      return
  ...
  async function handleClaim(messageId: string) {
    const message = threadState.status === 'ready' ? threadState.thread.messages.find((item) => item.id === messageId) : undefined
    if (!message) return
  ...
  async function handleAssignment(message: PatientMessageItem, requestedAssignee?: string, requestedReason?: string) {
    ...
    setAssignmentError(null)
    try {
      const updated = await updatePatientMessageAssignment(session.sessionId, message.id, {
      ...
      setReload((value) => value + 1)
      showToast(assignedTo ? `Message assigned to ${assignedTo}.` : 'Message assignment removed.', 'success')
    } catch (error) {
    ...
    <div className="clinician-page">
      <div className="clinician-page-header">
        ...
        <div className="clinician-header-actions">
          <button className="cl-btn-secondary" type="button" disabled={inboxLoading} onClick={() => setReload((value) => value + 1)}>
      ...
      {inbox && (
        <div className="message-inbox-counts" aria-label="Inbox counts">
          ...
          <button type="button" onClick={() => setQuickFilter('assignment', 'mine')}>
            <strong>{inbox.counts.assignedToMe}</strong>
            <span>Assigned to me</span>
          </button>
          <button type="button" onClick={() => setQuickFilter('assignment', 'unassigned')}>
            <strong>{inbox.counts.unassigned}</strong>
      ...
      <form className="cl-card message-inbox-filters" onSubmit={applyFilters}>
        ...
        <label className="cl-admin-field">
          <span>Owner</span>
          <input className="ne-input" value={draft.owner} onChange={(event) => setDraft((current) => ({ ...current, owner: event.target.value }))} />
        </label>
        <label className="cl-admin-field">
          ...
          <input
            ...
            onChange={(event) =>
              setDraft((current) => ({
                ...current,
                minimumAgeDays: event.target.value,
              }))
            }
          />
        </label>
      ...
      {composeMode && (
        <section className="ne-done cl-card" aria-labelledby="compose-message-title">
          <h2 id="compose-message-title" className="cl-card-title">
            Compose new message
          </h2>
          ...
          <div className="ne-actions">
            <button className="cl-btn-secondary" type="button" onClick={() => setComposeMode(false)}>
              Cancel
            </button>
        ...
        <section className="msg-sidebar" aria-label="Filtered message inbox" aria-busy={inboxLoading}>
          {inboxLoading && !inbox && (
            <div className="skeleton-list">
              {[0, 1, 2, 3].map((item) => (
                <div key={item} className="skeleton-row" style={{ height: 72 }} />
              ))}
            </div>
          )}
          {inbox && inbox.items.length === 0 && !inboxLoading && <p className="cl-empty-text">No messages match these filters.</p>}
          ...
          {inbox && (
            <div className="message-inbox-pagination">
              <button className="cl-btn-secondary" type="button" disabled={currentOffset === 0} onClick={() => changePage(Math.max(0, currentOffset - PAGE_SIZE))}>
                <ChevronLeft size={14} aria-hidden="true" />
                Previous
              </button>
              <span>
                {inbox.total === 0 ? '0' : `${currentOffset + 1}–${currentOffset + inbox.items.length}`} of {inbox.total}
              </span>
              <button className="cl-btn-secondary" type="button" disabled={!hasNextPage} onClick={() => changePage(currentOffset + PAGE_SIZE)}>
                Next
        ...
        <section className="msg-thread-panel" aria-label="Message thread">
          ...
          {threadState.status === 'error' && (
            <div className="error-banner" role="alert">
              {threadState.message}
              <button className="cl-btn-secondary" type="button" onClick={() => openThread(threadState.patient)}>
                Retry
              </button>
            </div>
          )}
            ...
            <>
              <div className="msg-thread-header">
                <div className="msg-thread-patient">
                  <div className="msg-patient-avatar msg-patient-avatar-lg">{initials(threadState.thread.patient.displayName)}</div>
                  <div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (threadState.status === 'ready') openThread(threadState.thread.patient, next)
                  }}
                >
                    ...
                    <article key={message.id} className="msg-item cl-card">
                      <div className="msg-item-header">
                        <div>
                          ...
                          <p className="msg-item-meta">
                            {message.date}
                            {message.assignedTo ? ` · Assigned: ${message.assignedTo}` : ' · Unassigned'}
                          </p>
                        </div>
                        <span className={`cl-badge ${messageStatusClass(message.status)}`}>{message.status ?? 'Unknown'}</span>
                        {message.deleted !== 0 && <span className="cl-badge cl-badge-red">Archived</span>}
                      </div>
                      {message.body && <p className="msg-item-body">{message.body}</p>}
                      {message.deleted !== 0 && (
                        <p className="cl-empty-text" role="status">
                          This archived message is read-only. Restore it with a retained reason before replying, forwarding, correcting, attaching files, or changing assignment.
                        </p>
                      )}
                      <div className="msg-reply-form">
                        <label className="ne-field">
                          <span className="ne-label">Attach file</span>
                          <input
                            className="ne-input"
                            type="file"
                            accept="application/pdf,image/png,image/jpeg,text/plain"
                            disabled={message.deleted !== 0}
                            onChange={(event) =>
                              setAttachmentFiles((current) => ({
                                ...current,
                                [message.id]: event.target.files?.[0] ?? null,
                              }))
                            }
                          />
                        </label>
                        <div className="ne-actions">
                          <button className="cl-btn-secondary" type="button" disabled={message.deleted !== 0 || !attachmentFiles[message.id] || attachmentBusyId !== null} onClick={() => void uploadAttachment(message.id)}>
                            <Paperclip size={14} />
                            {attachmentBusyId === message.id ? 'Uploading…' : 'Add attachment'}
                          </button>
                          <button className="cl-btn-secondary" type="button" onClick={() => void loadAttachments(message.id)}>
                            Show attachments
                          </button>
                        </div>
                        {attachments[message.id]?.map((attachment) => (
                          <div key={attachment.id} className="msg-item-meta">
                            {attachment.fileName} · {Math.ceil(attachment.sizeBytes / 1024)} KiB · {attachment.uploadedBy}{' '}
                            <button className="cl-link" type="button" onClick={() => void downloadAttachment(message.id, attachment)}>
                      ...
                      <div className="ne-actions">
                        {message.assignedTo === session.username ? (
                          ...
                          <span className="cl-badge cl-badge-muted">Assigned to {message.assignedTo}; use the reasoned assignment form below to reassign.</span>
                        ) : (
                          <button className="cl-btn-secondary" type="button" disabled={message.deleted !== 0 || assigningId !== null} onClick={() => void handleClaim(message.id)}>
                            {assigningId === message.id ? 'Assigning…' : 'Claim'}
                          </button>
                        )}
                        <button className="cl-btn-secondary" type="button" disabled={message.deleted !== 0 || forwardingId !== null || assignees.length === 0} onClick={() => setForwardingOpenId((current) => (current === message.id ? null : message.id))}>
                          <Forward size={14} aria-hidden="true" />
                          Forward
                        </button>
                        <button className="cl-btn-secondary" type="button" disabled={message.deleted !== 0 || correctionBusyId !== null} onClick={() => setCorrectionOpenId((current) => (current === message.id ? null : message.id))}>
                          Append correction
                        </button>
                        <button className="cl-btn-secondary" type="button" disabled={archiveBusyId !== null} onClick={() => setArchiveOpenId((current) => (current === message.id ? null : message.id))}>
                          {message.deleted !== 0 ? 'Restore' : 'Archive'}
                        </button>
                        <button className="cl-btn-secondary" type="button" disabled={message.deleted !== 0 || escalationBusyId !== null || escalationHistoryLoadingId !== null} onClick={() => void toggleEscalationPanel(message.id)}>
                          {escalationHistoryLoadingId === message.id ? 'Loading escalation…' : 'Escalation'}
                        </button>
                      </div>
                      {message.deleted === 0 && forwardingOpenId === message.id && (
                        <div className="msg-reply-form">
                          ...
                          <label className="ne-field">
                            ...
                            <select className="ne-input" value={forwardDrafts[message.id]?.assignedTo ?? ''} disabled={forwardingId !== null || assignees.length === 0} onChange={(event) => updateForwardDraft(message.id, 'assignedTo', event.target.value)}>
                              <option value="">Choose active staff</option>
                              {assignees
                                .filter((assignee) => assignee.username !== message.assignedTo)
                                .map((assignee) => (
                          ...
                          <label className="ne-field">
                            ...
                            <textarea className="ne-soap-textarea" rows={2} maxLength={500} value={forwardDrafts[message.id]?.note ?? ''} disabled={forwardingId !== null} onChange={(event) => updateForwardDraft(message.id, 'note', event.target.value)} />
                          </label>
                          <div className="ne-actions">
                            <button className="cl-btn-secondary" type="button" disabled={forwardingId !== null} onClick={() => setForwardingOpenId(null)}>
                              Cancel
                            </button>
                            <button className="cl-btn-primary" type="button" disabled={forwardingId !== null || !(forwardDrafts[message.id]?.assignedTo ?? '')} onClick={() => void handleForward(message)}>
                              <Forward size={14} aria-hidden="true" />
                      ...
                      {message.deleted === 0 && escalationOpenId === message.id && (
                        <div className="msg-reply-form">
                          ...
                          <label className="ne-field">
                            ...
                            <input
                              className="ne-input"
                              maxLength={500}
                              value={escalationReason[message.id] ?? ''}
                              disabled={escalationBusyId !== null}
                              onChange={(event) =>
                                setEscalationReason((current) => ({
                                  ...current,
                          ...
                          <div className="ne-actions">
                            <button className="cl-btn-secondary" type="button" disabled={escalationBusyId !== null} onClick={() => setEscalationOpenId(null)}>
                              Cancel
                            </button>
                            <button className="cl-btn-primary" type="button" disabled={escalationBusyId !== null || escalationHistoryLoadingId === message.id || escalationHistory[message.id]?.events[0]?.action === 'escalated' || !escalationReason[message.id]?.trim()} onClick={() => void handleEscalation(message, true)}>
                              {escalationBusyId === message.id ? 'Recording…' : 'Escalate'}
                            </button>
                            <button className="cl-btn-secondary" type="button" disabled={escalationBusyId !== null || escalationHistoryLoadingId === message.id || escalationHistory[message.id]?.events[0]?.action !== 'escalated' || !escalationReason[message.id]?.trim()} onClick={() => void handleEscalation(message, false)}>
                              Resolve escalation
                            </button>
                            <button className="cl-btn-secondary" type="button" disabled={escalationBusyId !== null || escalationHistoryLoadingId === message.id} onClick={() => void toggleEscalationHistory(message.id)}>
                              {escalationHistoryLoadingId === message.id ? 'Loading history…' : 'Refresh escalation history'}
                            </button>
                          </div>
                          {escalationHistory[message.id] && (
                            <ul className="message-inbox-list" aria-label="Escalation history">
                              {escalationHistory[message.id].events.length === 0 ? (
                                <li>No escalation events have been recorded.</li>
                              ) : (
                                escalationHistory[message.id].events.map((event) => (
                      ...
                      {message.deleted === 0 && correctionOpenId === message.id && (
                        <div className="msg-reply-form">
                          ...
                          <div className="ne-actions">
                            <button className="cl-btn-secondary" type="button" disabled={correctionBusyId !== null} onClick={() => setCorrectionOpenId(null)}>
                              Cancel
                            </button>
                            <button className="cl-btn-primary" type="button" disabled={correctionBusyId !== null || !correctionDrafts[message.id]?.correction.trim() || !correctionDrafts[message.id]?.reason.trim()} onClick={() => void handleCorrection(message)}>
                              {correctionBusyId === message.id ? 'Recording…' : 'Record correction'}
                            </button>
                            <button className="cl-btn-secondary" type="button" disabled={correctionBusyId !== null} onClick={() => void toggleCorrectionHistory(message.id)}>
                              {correctionHistory[message.id] ? 'Hide correction history' : 'Correction history'}
                            </button>
                          </div>
                          {correctionHistory[message.id] && (
                            <ul className="message-inbox-list" aria-label="Correction history">
                              {correctionHistory[message.id].events.length === 0 ? (
                                <li>No corrections have been recorded.</li>
                              ) : (
                                correctionHistory[message.id].events.map((event) => (
                                  <li key={event.eventId}>
                      ...
                      {archiveOpenId === message.id && (
                        <div className="msg-reply-form">
                          ...
                          <label className="ne-field">
                            <span className="ne-label">Archive reason</span>
                            <input
                              className="ne-input"
                              maxLength={500}
                              value={archiveReason[message.id] ?? ''}
                          ...
                          <div className="ne-actions">
                            <button className="cl-btn-secondary" type="button" disabled={archiveBusyId !== null} onClick={() => setArchiveOpenId(null)}>
                              Cancel
                            </button>
                            <button className="cl-btn-primary" type="button" disabled={archiveBusyId !== null || !archiveReason[message.id]?.trim()} onClick={() => void (message.deleted !== 0 ? restoreMessage(message) : archiveMessage(message))}>
                              {archiveBusyId === message.id ? (message.deleted !== 0 ? 'Restoring…' : 'Archiving…') : message.deleted !== 0 ? 'Restore message' : 'Archive message'}
                            </button>
                            <button className="cl-btn-secondary" type="button" disabled={archiveBusyId !== null} onClick={() => void toggleRetentionHistory(message.id)}>
                              {retentionHistory[message.id] ? 'Hide archive history' : 'Archive history'}
                            </button>
                          </div>
                          {retentionHistory[message.id] && (
                            <ul className="message-inbox-list" aria-label="Archive history">
                              {retentionHistory[message.id].events.map((event) => (
                                <li key={event.eventId}>
                                  <strong>{event.action}</strong>
                                  {' · '}
                                  {event.actor}
                      ...
                      <div className="msg-reply-form">
                        <label className="ne-field">
                          ...
                          <select className="ne-input" value={assignmentDrafts[message.id]?.assignedTo ?? message.assignedTo ?? ''} disabled={message.deleted !== 0 || assigningId !== null || assignees.length === 0} onChange={(event) => updateAssignmentDraft(message.id, 'assignedTo', event.target.value)}>
                        ...
                        <label className="ne-field">
                          <span className="ne-label">
                            Reason
                            {message.assignedTo ? ' (required to reassign or unassign)' : ' (optional when claiming)'}
                          </span>
                          <input className="ne-input" maxLength={500} value={assignmentDrafts[message.id]?.reason ?? ''} disabled={message.deleted !== 0 || assigningId !== null} onChange={(event) => updateAssignmentDraft(message.id, 'reason', event.target.value)} />
                        </label>
                        <div className="ne-actions">
                          <button className="cl-btn-secondary" type="button" disabled={message.deleted !== 0 || assigningId !== null || assignees.length === 0 || (assignmentDrafts[message.id]?.assignedTo ?? message.assignedTo ?? '') === (message.assignedTo ?? '')} onClick={() => void handleAssignment(message)}>
                            {assigningId === message.id ? 'Savingâ€¦' : 'Save assignment'}
                          </button>
                          <button className="cl-btn-secondary" type="button" disabled={historyLoadingId === message.id} onClick={() => void toggleAssignmentHistory(message.id)}>
                            {historyLoadingId === message.id ? 'Loading historyâ€¦' : assignmentHistory[message.id] ? 'Hide assignment history' : 'Assignment history'}
                          </button>
                        </div>
                        {assigneeError && (
                        ...
                        {assignmentHistory[message.id] && (
                          <div className="cl-card" aria-label="Assignment history">
                            <p className="msg-item-meta">Assignment version {assignmentHistory[message.id].currentVersion}</p>
