with corrected as (
  select r.definition_id,r.revision,
    jsonb_set(r.schema_json,'{fields,0,children,0,required}','false'::jsonb) as schema_json
...
select d.definition_id,2,'legacy-adoption-corrected','effective','effective','migration-v0117','Allow empty local Physical Exam repeat rows because legacy save behavior skips empty catalog rows.',now(),r.schema_hash
