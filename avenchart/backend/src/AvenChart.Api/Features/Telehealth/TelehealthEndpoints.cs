...
public static class TelehealthEndpoints
{
    ...
    public static WebApplication MapTelehealthEndpoints(this WebApplication app)
    {
        ...
        applicants.MapGet("/{applicantId:guid}/telehealth-request/{requestId:guid}/post-visit-receipt", GetApplicantSyntheticPostVisitReceiptAsync)
            ...
            .ProducesProblem(StatusCodes.Status401Unauthorized)
            .ProducesProblem(StatusCodes.Status404NotFound);
        applicants.MapGet("/{applicantId:guid}/telehealth-request/{requestId:guid}/after-visit-plan-preview", GetApplicantSyntheticAfterVisitPlanPreviewAsync)
            .WithName("GetTelehealthApplicantSyntheticAfterVisitPlanPreview")
            .WithDescription("Returns the exact access-key owner's immutable physician-authored NON_PRODUCTION after-visit plan preview. It is not medical advice, an after-visit summary, notification, or delivery.")
            .Produces<TelehealthSyntheticAfterVisitPlanPreviewResponse>()
            .ProducesProblem(StatusCodes.Status401Unauthorized)
            .ProducesProblem(StatusCodes.Status404NotFound);
        ...
        patient.MapGet("/requests/{requestId:guid}/post-visit-receipt", GetPatientSyntheticPostVisitReceiptAsync)
            ...
            .ProducesProblem(StatusCodes.Status401Unauthorized)
            .ProducesProblem(StatusCodes.Status404NotFound);
        patient.MapGet("/requests/{requestId:guid}/after-visit-plan-preview", GetPatientSyntheticAfterVisitPlanPreviewAsync)
            .WithName("GetPatientTelehealthSyntheticAfterVisitPlanPreview")
            .WithDescription("Returns the authenticated request owner's immutable physician-authored NON_PRODUCTION after-visit plan preview. It is not medical advice, an after-visit summary, notification, or delivery.")
            .Produces<TelehealthSyntheticAfterVisitPlanPreviewResponse>()
            .ProducesProblem(StatusCodes.Status401Unauthorized)
            .ProducesProblem(StatusCodes.Status404NotFound);
        ...
        clinician.MapGet("/consultations/{consultationId:guid}/professional-claim-preparation", GetProfessionalClaimPreparationAsync)
            ...
            .Produces<TelehealthProfessionalClaimPreparationWorkspaceResponse>()
            .ProducesProblem(StatusCodes.Status403Forbidden)
            .ProducesProblem(StatusCodes.Status404NotFound)
            .AddEndpointFilter(AccessPermissionFilter("patients", "demo", "view"))
            .AddEndpointFilter(AccessPermissionFilter("encounters", "auth", "view"));
    ...
    private static Task<IResult> GetApplicantSyntheticAfterVisitPlanPreviewAsync(
        TelehealthSyntheticAfterVisitPlanPreviewService service,
        HttpContext context,
        Guid applicantId,
        Guid requestId,
        CancellationToken cancellationToken) =>
    ...
    private static Task<IResult> GetPatientSyntheticAfterVisitPlanPreviewAsync(
        TelehealthSyntheticAfterVisitPlanPreviewService service,
        HttpContext context,
        Guid requestId,
        CancellationToken cancellationToken) =>
