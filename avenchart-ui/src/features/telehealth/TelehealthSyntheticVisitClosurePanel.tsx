...
import { closeSyntheticTelehealthVisit, type TelehealthSyntheticVisitClosure } from './api.ts'

export default function TelehealthSyntheticVisitClosurePanel({ consultationId, expectedVersion, onClosed }: { consultationId: string, expectedVersion: number, onClosed: (result: TelehealthSyntheticVisitClosure) => void }) {
  ...
  async function close() {
    ...
    const input = `${consultationId}:${expectedVersion}`
    if (commandKey.current?.input !== input) commandKey.current = { input, key: crypto.randomUUID() }
    try {
      const result = await closeSyntheticTelehealthVisit(consultationId, { expectedConsultationVersion: expectedVersion, encounterLockReviewed: true, syntheticClosureConfirmed: true }, undefined, commandKey.current.key)
      setMessage(result.clinicianAvailableForNewWork ? 'Synthetic visit closed. The doctor is available for new work; the appointment, delivery, billing, claims, and integrations remain unchanged.' : 'Synthetic visit closed. The appointment, delivery, billing, claims, and integrations remain unchanged. Confirm the doctor\'s current availability before assigning new work.')
      onClosed(result)
    } catch (caught) {
      setMessage(caught instanceof Error ? caught.message : 'Closure was not confirmed. Retry unchanged to recover an interrupted response.')
