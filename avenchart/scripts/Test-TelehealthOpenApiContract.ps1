...
try {
    ...
    $expected = @(
      ...
      '/api/telehealth/v1/patient/requests/{requestId}/history','/api/telehealth/v1/patient/requests/{requestId}/status','/api/telehealth/v1/patient/requests/{requestId}/readiness','/api/telehealth/v1/patient/requests/{requestId}/coverage/verify','/api/telehealth/v1/patient/requests/{requestId}/cancel',
      '/api/telehealth/v1/patient/requests/{requestId}/connection-grants',
      '/api/telehealth/v1/patient/requests/{requestId}/conversation',
      ...
      '/api/telehealth/v1/clinician/consultations/{consultationId}/conversation',
    ...
    $patientPostVisitReceipt = Get-Operation $document '/api/telehealth/v1/patient/requests/{requestId}/post-visit-receipt' 'get'
    $patientPostVisitReceiptReference = Get-Property (Get-Property (Get-Property (Get-Property $patientPostVisitReceipt.responses '200') 'content') 'application/json').schema '$ref'
    $patientPostVisitReceiptSchemaName = ($patientPostVisitReceiptReference -split '/')[-1]
    $patientPostVisitReceiptJson = ((Get-Property $document.components 'schemas').$patientPostVisitReceiptSchemaName | ConvertTo-Json -Depth 20 -Compress)
    Add-Check 'Synthetic post-visit receipt is patient-authenticated, read-only, and minimized' (
      (Has-Security $patientPostVisitReceipt 'AvenChartPatientPortalSession') -and (Has-Security $patientPostVisitReceipt 'AvenChartOidcBearer') -and
      -not (Has-Security $patientPostVisitReceipt 'AvenChartLocalStaffSession') -and -not (Has-Header $patientPostVisitReceipt 'X-Idempotency-Key') -and
      $null-eq(Get-Property $patientPostVisitReceipt 'requestBody') -and $null-ne(Get-Property $patientPostVisitReceipt.responses '401') -and $null-ne(Get-Property $patientPostVisitReceipt.responses '404') -and
      $patientPostVisitReceiptJson-match'receiptState' -and $patientPostVisitReceiptJson-match'sourceMode' -and $patientPostVisitReceiptJson-match'syntheticDataConfirmed' -and
      $patientPostVisitReceiptJson-match'appointmentCompleted' -and $patientPostVisitReceiptJson-match'encounterCompleted' -and $patientPostVisitReceiptJson-match'clinicalRecordDelivered' -and
      $patientPostVisitReceiptJson-match'prescriptionDelivered' -and $patientPostVisitReceiptJson-match'billingCreated' -and $patientPostVisitReceiptJson-match'claimCreated' -and
      $patientPostVisitReceiptJson-match'notificationSent' -and $patientPostVisitReceiptJson-match'externalDestinationContacted' -and
      $patientPostVisitReceiptJson-notmatch'patientId|encounterId|appointmentId|physician|diagnosis|medication|pharmacy|insurance|claimId|billingId|instruction|followUp')
    $applicantPostVisitReceipt = Get-Operation $document '/api/telehealth/v1/applicants/{applicantId}/telehealth-request/{requestId}/post-visit-receipt' 'get'
    Add-Check 'Synthetic post-visit receipt is applicant-key authenticated, read-only, and excludes patient or staff identity' (
      (Has-Security $applicantPostVisitReceipt 'AvenChartTelehealthApplicantAccess') -and
      -not (Has-Security $applicantPostVisitReceipt 'AvenChartPatientPortalSession') -and
      -not (Has-Security $applicantPostVisitReceipt 'AvenChartLocalStaffSession') -and
      -not (Has-Header $applicantPostVisitReceipt 'X-Idempotency-Key') -and
    ...
    $applicantQueuedWithdrawal = Get-Operation $document '/api/telehealth/v1/applicants/{applicantId}/telehealth-request/{requestId}/withdraw' 'post'
    $applicantQueuedWithdrawalPath = Get-Property (Get-Property $document 'paths') '/api/telehealth/v1/applicants/{applicantId}/telehealth-request/{requestId}/withdraw'
    $applicantQueuedWithdrawalRequestReference = Get-Property (Get-Property (Get-Property (Get-Property $applicantQueuedWithdrawal 'requestBody') 'content') 'application/json').schema '$ref'
    $applicantQueuedWithdrawalRequestSchema = Get-Property (Get-Property (Get-Property $document 'components') 'schemas') (($applicantQueuedWithdrawalRequestReference -split '/')[-1])
    $applicantQueuedWithdrawalRequestProperties = @((Get-Property $applicantQueuedWithdrawalRequestSchema 'properties').PSObject.Properties.Name)
    ...
    Add-Check 'Applicant queued withdrawal is access-key-only, idempotent, explicitly confirmed, and consequence-minimized' (
      ...
      -not (Has-Security $applicantQueuedWithdrawal 'AvenChartLocalStaffSession') -and
      (Has-Header $applicantQueuedWithdrawal 'X-Idempotency-Key') -and
      $null-ne(Get-Property $applicantQueuedWithdrawal 'requestBody') -and
      $null-ne(Get-Property $applicantQueuedWithdrawal.responses '400') -and
      $null-ne(Get-Property $applicantQueuedWithdrawal.responses '401') -and
      $null-ne(Get-Property $applicantQueuedWithdrawal.responses '404') -and
      $null-ne(Get-Property $applicantQueuedWithdrawal.responses '409') -and
      $null-ne(Get-Property $applicantQueuedWithdrawal.responses '410') -and
      @(Compare-Object @('expectedRequestVersion','syntheticWithdrawalConfirmed') ($applicantQueuedWithdrawalRequestProperties|Sort-Object)).Count -eq 0 -and
      @(Compare-Object @('connectionCreated','consultationCreated','externalActionPerformed','limitations','provisionalAppointmentCancelled','queueEntryRemoved','requestId','requestStatus','requestVersion','reservationCreated','sourceMode','withdrawnAt') ($applicantQueuedWithdrawalResponseProperties|Sort-Object)).Count -eq 0 -and
      @($applicantQueuedWithdrawalRequestProperties + $applicantQueuedWithdrawalResponseProperties | Where-Object { $_ -match 'applicantId|patientId|canonicalPatientId|accessKey|provider(Id|Name)?|doctor(Id|Name)?|physician(Id|Name)?|npi|payer|diagnosis|prescription(Id|Details)|claim(Id|Details)|encounter(Id|Details)|consultation(Id|Details)|note|freeText|fingerprint' }).Count -eq 0)
