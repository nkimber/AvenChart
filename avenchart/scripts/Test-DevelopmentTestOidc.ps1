...
function Add-Check {
    ...
    $checkStatus = if ($Passed) { "passed" } else { "failed" }
    $script:checks.Add([ordered]@{ name = $Name; status = $checkStatus; details = $Details })
...
function Get-QueryValues {
    ...
    foreach ($pair in $query.Split('&', [System.StringSplitOptions]::RemoveEmptyEntries)) {
        ...
        if ($parts.Count -gt 1) { $value = [uri]::UnescapeDataString($parts[1]) } else { $value = '' }
        $values[$name] = $value
    }
    return $values
}
...
function Start-BrowserOidcFlow {
    ...
    if ($callback.StatusCode -ne 302) {
        throw "AvenChart did not complete the browser OIDC callback for $Audience (HTTP $($callback.StatusCode)): $($callback.Content)"
    }
...
try {
    ...
    $facility = @($session.accessContext.facilities | Where-Object { $_.isDefault } | Select-Object -First 1)
    $purpose = @($session.accessContext.purposes | Select-Object -First 1)
    if ($facility.Count -ne 1 -or $purpose.Count -ne 1) { throw 'The mapped test OIDC principal has no usable facility/purpose context.' }
    $staffHeaders = @{
        Authorization = $bearerHeaders.Authorization
        'X-AvenChart-Facility-Id' = [string]$facility[0].facilityId
        'X-AvenChart-Purpose-Of-Use' = [string]$purpose[0]
    }
    $mappings = Invoke-RestMethod -Uri "$ApiBaseUrl/api/administration/external-identity-mappings?providerId=test-oidc" -Headers $staffHeaders -TimeoutSec 20
    ...
    $mappingId = if ($mapping.Count -eq 1) { $mapping[0].mappingId } else { $null }
    Add-Check "Test OIDC token resolves through an explicit active provider-subject mapping" ($mapping.Count -eq 1) @{ mappingId = $mappingId }
    ...
    $portalMappings = @(Invoke-RestMethod -Uri "$ApiBaseUrl/api/administration/patient-portal-external-identity-mappings?providerId=test-oidc" -Headers $staffHeaders -TimeoutSec 20 | ForEach-Object { $_ })
    ...
    if ($portalMapping.Count -eq 0) {
        $portalMapping = @(
            Invoke-RestMethod -Uri "$ApiBaseUrl/api/administration/patient-portal-external-identity-mappings" -Method Post -Headers $staffHeaders -ContentType 'application/json' `
    ...
    Invoke-RestMethod -Uri "$ApiBaseUrl/api/administration/patient-portal-external-identity-mappings/$($portalMapping[0].mappingId)/deactivate" -Method Post -Headers $staffHeaders -ContentType 'application/json' `
    ...
    $invalidSession = Invoke-RestMethod -Uri "$ApiBaseUrl/api/auth/session" -Headers @{ Authorization = 'Bearer not-a-jwt' } -TimeoutSec 20
    ...
    Add-Check "Revoked portal mapping, invalid bearer, and disabled local-login boundaries fail closed" (
        ...
        -not $invalidSession.authenticated -and
        $localLoginStatus -eq 404 -and
        $localPortalLoginStatus -eq 404
    ) @{ revokedPortalAuthenticated = $revokedPortalSession.authenticated; invalidBearerAuthenticated = $invalidSession.authenticated; localLogin = $localLoginStatus; localPortalLogin = $localPortalLoginStatus }
}
catch {
