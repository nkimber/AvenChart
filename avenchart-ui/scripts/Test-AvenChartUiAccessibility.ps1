...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AvenChart schema migrations failed before the accessibility gate."
    }
...
if ($exitCode -ne 0) {
    throw "AvenChart UI accessibility gate failed. See $ResultPath."
}

Write-Host "AvenChart UI accessibility gate passed: $ResultPath"
