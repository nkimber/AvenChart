...
try {
    ...
    try { docker compose exec -T postgres psql -U legacy-ehr -d legacy-ehr_modernized -v ON_ERROR_STOP=1 -c $query | Out-Null; if ($LASTEXITCODE -ne 0) { throw "Expired dispatch lease setup failed." } }
    ...
    Push-Location $solutionRoot
    try { $audit = docker compose exec -T postgres psql -U legacy-ehr -d legacy-ehr_modernized -t -A -c "select action || ':' || actor || ':' || attempt_count from integration_outbox_events where event_id='$($queued.eventId)' order by occurred_at;" }
    finally { Pop-Location }
