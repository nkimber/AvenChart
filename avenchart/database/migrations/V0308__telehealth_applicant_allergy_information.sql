...
-- Adds one immutable, patient-reported synthetic allergy/intolerance-information
-- receipt and zero or more immutable selections from a fixed local catalog.
-- This is not an AllergyIntolerance resource, canonical allergy list, confirmed
-- negation, reconciliation, contraindication check, alert, or care capability.
...
create table if not exists telehealth_applicant_allergy_information_receipts (
  ...
  inventory_allergies_or_intolerances_status text not null,
  ...
  no_reaction_or_criticality_captured_acknowledged boolean not null,
  ...
  allergy_intolerance_created boolean not null default false,
  allergy_list_reconciled boolean not null default false,
  reaction_assessed boolean not null default false,
  criticality_assessed boolean not null default false,
  contraindication_check_performed boolean not null default false,
  ...
  constraint uq_telehealth_allergy_information_practice_key
    unique(practice_id,idempotency_key),
  constraint chk_telehealth_allergy_information_result check (
    resulting_applicant_version > 0
    and resulting_applicant_status='SyntheticAllergyInformationRecorded'),
  constraint chk_telehealth_allergy_information_hashes check (
    allergy_information_snapshot_fingerprint ~ '^[0-9a-f]{64}$'
    and medication_information_snapshot_fingerprint ~ '^[0-9a-f]{64}$'
    and command_fingerprint ~ '^[0-9a-f]{64}$'),
  constraint chk_telehealth_allergy_information_inventory_status check (
    inventory_allergies_or_intolerances_status in ('PatientReportsNone','ItemsToReview','Unsure')),
  constraint chk_telehealth_allergy_information_count check (
    selected_item_count between 0 and 6),
  constraint chk_telehealth_allergy_information_branch check (
    (inventory_allergies_or_intolerances_status='ItemsToReview')
    or (selected_item_count=0 and not additional_or_unlisted_items_reported)),
  constraint chk_telehealth_allergy_information_route check (
    review_route = case
      when additional_or_unlisted_items_reported
        then 'AdditionalAllergyCollectionRequired'
      when inventory_allergies_or_intolerances_status='ItemsToReview'
        then 'ClinicianAllergyReviewRequired'
      when inventory_allergies_or_intolerances_status='Unsure'
        then 'AssistedAllergyReviewRequired'
      else 'PendingClinicianConfirmationOfPatientReportedNone'
    end),
  constraint chk_telehealth_allergy_information_acknowledgments check (
    patient_reported_may_be_incomplete_acknowledged
    and synthetic_catalog_incomplete_acknowledged
    and no_reaction_or_criticality_captured_acknowledged
    and clinician_verification_required_acknowledged),
  constraint chk_telehealth_allergy_information_catalog check (
    catalog_key='avenchart-synthetic-applicant-allergy-substances-2026-08'
    and catalog_version=1
    and coding_system='LOCAL_SYNTHETIC_ONLY'
    and not catalog_complete),
  constraint chk_telehealth_allergy_information_policy check (
    policy_key='SYNTHETIC_APPLICANT_ALLERGY_INFORMATION'
    and policy_version=1
    and evidence_type='PROMOTED_PATIENT_ALLERGY_INFORMATION_RECEIPT'),
  constraint chk_telehealth_allergy_information_expiry check (
    recorded_at <= applicant_expires_at),
  constraint chk_telehealth_allergy_information_no_consequence check (
    not allergy_intolerance_created
    and not allergy_list_reconciled
    and not reaction_assessed
    and not criticality_assessed
    and not contraindication_check_performed
...
create table if not exists telehealth_applicant_reported_allergy_items (
  ...
  recorded_at timestamptz not null default now(),
  constraint uq_telehealth_reported_allergy_item_ordinal
    unique(receipt_id,item_ordinal),
  constraint uq_telehealth_reported_allergy_item_catalog
    unique(receipt_id,catalog_key),
  constraint chk_telehealth_reported_allergy_item_ordinal check (
    item_ordinal between 1 and 6),
  constraint chk_telehealth_reported_allergy_item_catalog check (
...
create or replace function enforce_telehealth_applicant_allergy_information()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  if applicant_row.applicant_id is null
    ...
    raise exception using errcode='23514',
      message='telehealth_allergy_information_applicant_mismatch';
  end if;
  ...
  if promotion_row.applicant_id<>new.applicant_id
     ...
     or exists(select 1 from allergies allergy
               where lower(allergy.patient_id)=lower(new.canonical_patient_id)) then
    raise exception using errcode='23514',
      message='telehealth_allergy_information_patient_mismatch';
  end if;
  ...
  if medication_row.receipt_id is null
     ...
     or medication_row.resulting_applicant_version<>new.resulting_applicant_version-1
    ...
    raise exception using errcode='23514',
      message='telehealth_allergy_information_medication_mismatch';
  end if;
  ...
  if inventory_row.inventory_id is null
     ...
     or inventory_row.allergies_or_intolerances_status<>new.inventory_allergies_or_intolerances_status
     or inventory_row.allergies_or_intolerances_status not in ('PatientReportsNone','ItemsToReview','Unsure')
     ...
     or inventory_row.allergy_list_reconciled
    ...
    raise exception using errcode='23514',
      message='telehealth_allergy_information_inventory_mismatch';
  end if;
...
create or replace function enforce_telehealth_reported_allergy_item_provenance()
returns trigger
language plpgsql
as $$
declare
  receipt_row telehealth_applicant_allergy_information_receipts%rowtype;
begin
  ...
  if receipt_row.receipt_id is null
    ...
    raise exception using errcode='23514',
      message='telehealth_reported_allergy_item_provenance_mismatch';
  end if;
  return new;
end;
$$;

create or replace function enforce_telehealth_allergy_information_item_count()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  if actual_count<>receipt_row.selected_item_count
     or (receipt_row.inventory_allergies_or_intolerances_status='ItemsToReview'
         and actual_count=0
         and not receipt_row.additional_or_unlisted_items_reported)
     or (receipt_row.inventory_allergies_or_intolerances_status<>'ItemsToReview'
         and actual_count<>0) then
    raise exception using errcode='23514',
      message='telehealth_allergy_information_item_count_mismatch';
  end if;
  return null;
end;
$$;

drop trigger if exists trg_telehealth_allergy_information_guard
  on telehealth_applicant_allergy_information_receipts;
create trigger trg_telehealth_allergy_information_guard
before insert on telehealth_applicant_allergy_information_receipts
for each row execute function enforce_telehealth_applicant_allergy_information();

drop trigger if exists trg_telehealth_reported_allergy_item_guard
  on telehealth_applicant_reported_allergy_items;
create trigger trg_telehealth_reported_allergy_item_guard
before insert on telehealth_applicant_reported_allergy_items
for each row execute function enforce_telehealth_reported_allergy_item_provenance();

drop trigger if exists trg_telehealth_allergy_information_count_parent
  on telehealth_applicant_allergy_information_receipts;
create constraint trigger trg_telehealth_allergy_information_count_parent
after insert on telehealth_applicant_allergy_information_receipts
deferrable initially deferred
for each row execute function enforce_telehealth_allergy_information_item_count();

drop trigger if exists trg_telehealth_allergy_information_count_items
  on telehealth_applicant_reported_allergy_items;
create constraint trigger trg_telehealth_allergy_information_count_items
after insert or update or delete on telehealth_applicant_reported_allergy_items
deferrable initially deferred
for each row execute function enforce_telehealth_allergy_information_item_count();

drop trigger if exists trg_telehealth_allergy_information_append_only
  on telehealth_applicant_allergy_information_receipts;
create trigger trg_telehealth_allergy_information_append_only
before update or delete on telehealth_applicant_allergy_information_receipts
for each row execute function reject_telehealth_evidence_mutation();

drop trigger if exists trg_telehealth_reported_allergy_item_append_only
  on telehealth_applicant_reported_allergy_items;
create trigger trg_telehealth_reported_allergy_item_append_only
before update or delete on telehealth_applicant_reported_allergy_items
for each row execute function reject_telehealth_evidence_mutation();

create index if not exists ix_telehealth_allergy_information_practice
  on telehealth_applicant_allergy_information_receipts(
    practice_id,facility_id,recorded_at);

create index if not exists ix_telehealth_reported_allergy_items_receipt
  on telehealth_applicant_reported_allergy_items(receipt_id,item_ordinal);
