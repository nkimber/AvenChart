...
-- Adds one immutable, patient-reported synthetic health-history-topic receipt
-- and zero or more immutable selections from a fixed local catalog. Topics are
-- not diagnoses, findings, assessments, canonical history, or care capability.
...
alter table telehealth_applicant_events
  add constraint chk_telehealth_applicant_event_status check (
    (from_status is null or from_status in (
      ...
      'SyntheticDevicePreparationRecorded',
      'SyntheticClinicalInformationInventoryRecorded',
      'SyntheticMedicationInformationRecorded','SyntheticAllergyInformationRecorded',
      'SyntheticHealthHistoryInformationRecorded',
      'VerificationLocked','Expired'))
    and to_status in (
      'ContactVerificationPending','IdentityReviewPending',
...
create table if not exists telehealth_applicant_health_history_information_receipts (
  ...
  inventory_other_health_history_status text not null,
  selected_topic_count integer not null,
  additional_or_unlisted_topics_reported boolean not null,
  review_route text not null,
  patient_reported_may_be_incomplete_acknowledged boolean not null,
  topic_selection_is_not_diagnosis_acknowledged boolean not null,
  no_status_or_timing_captured_acknowledged boolean not null,
  clinician_verification_required_acknowledged boolean not null,
  ...
  procedure_created boolean not null default false,
  observation_created boolean not null default false,
  family_member_history_created boolean not null default false,
  questionnaire_response_created boolean not null default false,
  risk_modifier_evaluated boolean not null default false,
  ...
  constraint uq_telehealth_health_history_information_practice_key
    unique(practice_id,idempotency_key),
  constraint chk_telehealth_health_history_information_result check (
    resulting_applicant_version > 0
    and resulting_applicant_status='SyntheticHealthHistoryInformationRecorded'),
  constraint chk_telehealth_health_history_information_hashes check (
    health_history_information_snapshot_fingerprint ~ '^[0-9a-f]{64}$'
  ...
  constraint chk_telehealth_health_history_information_inventory_status check (
    inventory_other_health_history_status in ('PatientReportsNone','ItemsToReview','Unsure')),
  constraint chk_telehealth_health_history_information_count check (
    selected_topic_count between 0 and 6),
  constraint chk_telehealth_health_history_information_branch check (
    (inventory_other_health_history_status='ItemsToReview')
    or (selected_topic_count=0 and not additional_or_unlisted_topics_reported)),
  constraint chk_telehealth_health_history_information_route check (
    review_route = case
      when additional_or_unlisted_topics_reported
        then 'AdditionalHealthHistoryCollectionRequired'
      when inventory_other_health_history_status='ItemsToReview'
        then 'ClinicianHealthHistoryReviewRequired'
      when inventory_other_health_history_status='Unsure'
        then 'AssistedHealthHistoryReviewRequired'
      else 'PendingClinicianConfirmationOfPatientReportedNone'
    end),
  constraint chk_telehealth_health_history_information_acknowledgments check (
    patient_reported_may_be_incomplete_acknowledged
    and topic_selection_is_not_diagnosis_acknowledged
    and no_status_or_timing_captured_acknowledged
    and clinician_verification_required_acknowledged),
  constraint chk_telehealth_health_history_information_catalog check (
    catalog_key='avenchart-synthetic-applicant-health-history-topics-2026-08'
    and catalog_version=1
    and coding_system='LOCAL_SYNTHETIC_ONLY'
    and not catalog_complete),
  constraint chk_telehealth_health_history_information_policy check (
    policy_key='SYNTHETIC_APPLICANT_HEALTH_HISTORY_INFORMATION'
    and policy_version=1
    and evidence_type='PROMOTED_PATIENT_HEALTH_HISTORY_INFORMATION_RECEIPT'),
  constraint chk_telehealth_health_history_information_expiry check (
    recorded_at <= applicant_expires_at),
  constraint chk_telehealth_health_history_information_no_consequence check (
    not condition_created
    and not health_history_reconciled
    and not procedure_created
    and not observation_created
    and not family_member_history_created
    and not questionnaire_response_created
    and not clinician_review_created
    and not risk_modifier_evaluated
...
create table if not exists telehealth_applicant_reported_health_history_topics (
  ...
  catalog_version integer not null,
  coding_system text not null,
  snomed_ct_mapped boolean not null default false,
  icd10_cm_mapped boolean not null default false,
  loinc_mapped boolean not null default false,
  recorded_at timestamptz not null default now(),
  constraint uq_telehealth_reported_health_history_topic_ordinal
    unique(receipt_id,topic_ordinal),
  constraint uq_telehealth_reported_health_history_topic_catalog
    unique(receipt_id,catalog_key),
  constraint chk_telehealth_reported_health_history_topic_ordinal check (
    topic_ordinal between 1 and 6),
  constraint chk_telehealth_reported_health_history_topic_catalog check (
    (catalog_key,display_name,category) in (
    ...
    and catalog_version=1
    and coding_system='LOCAL_SYNTHETIC_ONLY'
    and not snomed_ct_mapped
    and not icd10_cm_mapped
    and not loinc_mapped)
);

create or replace function enforce_telehealth_applicant_health_history_information()
returns trigger
language plpgsql
as $$
declare
  ...
  medication_item_count integer;
  allergy_item_count integer;
begin
  ...
  if applicant_row.applicant_id is null
    ...
    raise exception using errcode='23514',
      message='telehealth_health_history_information_applicant_mismatch';
  end if;
  ...
  if promotion_row.applicant_id<>new.applicant_id
     ...
     or exists(select 1 from prescriptions prescription
               where lower(prescription.patient_id)=lower(new.canonical_patient_id))
     or exists(select 1 from allergies allergy
               where lower(allergy.patient_id)=lower(new.canonical_patient_id))
     or exists(select 1 from problems problem
               where lower(problem.patient_id)=lower(new.canonical_patient_id)) then
    raise exception using errcode='23514',
      message='telehealth_health_history_information_patient_mismatch';
  end if;

  if medication_row.receipt_id is null
     or medication_row.applicant_id<>new.applicant_id
     or medication_row.practice_id<>new.practice_id
     or medication_row.facility_id<>new.facility_id
     or medication_row.promotion_id<>new.promotion_id
     or medication_row.canonical_patient_id<>new.canonical_patient_id
     or medication_row.clinical_inventory_id<>new.clinical_inventory_id
     or medication_row.registration_details_confirmation_id<>new.registration_details_confirmation_id
     or medication_row.insurance_handoff_confirmation_id<>new.insurance_handoff_confirmation_id
     or medication_row.safety_evaluation_id<>new.safety_evaluation_id
     or medication_row.communication_access_readiness_id<>new.communication_access_readiness_id
     or medication_row.device_preparation_id<>new.device_preparation_id
     or medication_row.resulting_applicant_version<>new.resulting_applicant_version-2
     or medication_row.resulting_applicant_status<>'SyntheticMedicationInformationRecorded'
     or medication_row.medication_information_snapshot_fingerprint<>new.medication_information_snapshot_fingerprint
     or medication_row.policy_key<>'SYNTHETIC_APPLICANT_MEDICATION_INFORMATION'
     or medication_row.policy_version<>1
     or medication_row.evidence_type<>'PROMOTED_PATIENT_MEDICATION_INFORMATION_RECEIPT'
     or medication_row.catalog_key<>'avenchart-synthetic-applicant-medication-ingredients-2026-08'
     or medication_row.catalog_version<>1
     or medication_row.coding_system<>'LOCAL_SYNTHETIC_ONLY'
     or medication_row.catalog_complete
     or medication_item_count<>medication_row.selected_item_count
     or medication_row.medication_statement_created
     or medication_row.medication_request_created
     or medication_row.medication_list_reconciled
     or medication_row.interaction_check_performed
     or medication_row.clinician_review_created
     or medication_row.clinical_intake_completed
     or medication_row.clinical_eligibility_established
     or medication_row.patient_record_changed
     or medication_row.request_created
     or medication_row.queue_entered
     or medication_row.care_authorized
     or medication_row.prescribing_enabled then
    raise exception using errcode='23514',
      message='telehealth_health_history_information_medication_mismatch';
  end if;

  if allergy_row.receipt_id is null
     or allergy_row.applicant_id<>new.applicant_id
     or allergy_row.practice_id<>new.practice_id
     or allergy_row.facility_id<>new.facility_id
     or allergy_row.promotion_id<>new.promotion_id
     or allergy_row.canonical_patient_id<>new.canonical_patient_id
     ...
     or allergy_row.registration_details_confirmation_id<>new.registration_details_confirmation_id
     or allergy_row.insurance_handoff_confirmation_id<>new.insurance_handoff_confirmation_id
     or allergy_row.safety_evaluation_id<>new.safety_evaluation_id
     or allergy_row.communication_access_readiness_id<>new.communication_access_readiness_id
     or allergy_row.device_preparation_id<>new.device_preparation_id
     or allergy_row.resulting_applicant_version<>new.resulting_applicant_version-1
     or allergy_row.resulting_applicant_status<>'SyntheticAllergyInformationRecorded'
     or allergy_row.allergy_information_snapshot_fingerprint<>new.allergy_information_snapshot_fingerprint
     or allergy_row.medication_information_snapshot_fingerprint<>new.medication_information_snapshot_fingerprint
     or allergy_row.policy_key<>'SYNTHETIC_APPLICANT_ALLERGY_INFORMATION'
     or allergy_row.policy_version<>1
     or allergy_row.evidence_type<>'PROMOTED_PATIENT_ALLERGY_INFORMATION_RECEIPT'
     or allergy_row.catalog_key<>'avenchart-synthetic-applicant-allergy-substances-2026-08'
     or allergy_row.catalog_version<>1
     or allergy_row.coding_system<>'LOCAL_SYNTHETIC_ONLY'
     or allergy_row.catalog_complete
     or allergy_item_count<>allergy_row.selected_item_count
     or allergy_row.allergy_intolerance_created
     or allergy_row.allergy_list_reconciled
     or allergy_row.reaction_assessed
     or allergy_row.criticality_assessed
     or allergy_row.contraindication_check_performed
     or allergy_row.clinician_review_created
     or allergy_row.clinical_intake_completed
     or allergy_row.clinical_eligibility_established
     or allergy_row.patient_record_changed
     or allergy_row.request_created
     or allergy_row.queue_entered
     or allergy_row.care_authorized
     or allergy_row.prescribing_enabled then
    raise exception using errcode='23514',
      message='telehealth_health_history_information_allergy_mismatch';
  end if;
  ...
  if inventory_row.inventory_id is null
     ...
     or inventory_row.other_health_history_status<>new.inventory_other_health_history_status
     or inventory_row.other_health_history_status not in ('PatientReportsNone','ItemsToReview','Unsure')
     ...
     or inventory_row.health_history_reconciled
    ...
    raise exception using errcode='23514',
      message='telehealth_health_history_information_inventory_mismatch';
  end if;
...
create or replace function enforce_telehealth_reported_health_history_topic_provenance()
returns trigger
language plpgsql
as $$
declare
  receipt_row telehealth_applicant_health_history_information_receipts%rowtype;
begin
  ...
  if receipt_row.receipt_id is null
    ...
    raise exception using errcode='23514',
      message='telehealth_reported_health_history_topic_provenance_mismatch';
  end if;
  return new;
end;
$$;

create or replace function enforce_telehealth_health_history_information_topic_count()
returns trigger
language plpgsql
as $$
declare
...
begin
  ...
  where receipt_id=target_receipt_id;
  if actual_count<>receipt_row.selected_topic_count
     or (receipt_row.inventory_other_health_history_status='ItemsToReview'
         and actual_count=0
         and not receipt_row.additional_or_unlisted_topics_reported)
     or (receipt_row.inventory_other_health_history_status<>'ItemsToReview'
         and actual_count<>0) then
    raise exception using errcode='23514',
      message='telehealth_health_history_information_topic_count_mismatch';
  end if;
  return null;
end;
$$;

drop trigger if exists trg_telehealth_health_history_information_guard
  on telehealth_applicant_health_history_information_receipts;
create trigger trg_telehealth_health_history_information_guard
before insert on telehealth_applicant_health_history_information_receipts
for each row execute function enforce_telehealth_applicant_health_history_information();

drop trigger if exists trg_telehealth_reported_health_history_topic_guard
  on telehealth_applicant_reported_health_history_topics;
create trigger trg_telehealth_reported_health_history_topic_guard
before insert on telehealth_applicant_reported_health_history_topics
for each row execute function enforce_telehealth_reported_health_history_topic_provenance();

drop trigger if exists trg_telehealth_health_history_information_count_parent
  on telehealth_applicant_health_history_information_receipts;
create constraint trigger trg_telehealth_health_history_information_count_parent
after insert on telehealth_applicant_health_history_information_receipts
deferrable initially deferred
for each row execute function enforce_telehealth_health_history_information_topic_count();

drop trigger if exists trg_telehealth_health_history_information_count_items
  on telehealth_applicant_reported_health_history_topics;
create constraint trigger trg_telehealth_health_history_information_count_items
after insert or update or delete on telehealth_applicant_reported_health_history_topics
deferrable initially deferred
for each row execute function enforce_telehealth_health_history_information_topic_count();

drop trigger if exists trg_telehealth_health_history_information_append_only
  on telehealth_applicant_health_history_information_receipts;
create trigger trg_telehealth_health_history_information_append_only
before update or delete on telehealth_applicant_health_history_information_receipts
for each row execute function reject_telehealth_evidence_mutation();

drop trigger if exists trg_telehealth_reported_health_history_topic_append_only
  on telehealth_applicant_reported_health_history_topics;
create trigger trg_telehealth_reported_health_history_topic_append_only
before update or delete on telehealth_applicant_reported_health_history_topics
for each row execute function reject_telehealth_evidence_mutation();

create index if not exists ix_telehealth_health_history_information_practice
  on telehealth_applicant_health_history_information_receipts(
    practice_id,facility_id,recorded_at);

create index if not exists ix_telehealth_reported_health_history_topics_receipt
  on telehealth_applicant_reported_health_history_topics(receipt_id,topic_ordinal);
