...
insert into module_catalog(module_key,display_name,category,status,description,updated_at,updated_by) values
  ...
  ('DASHBOARD_CONTEXT','Dashboard Context Manager','clinical-workflow','decision-required','Legacy custom-module source `oe-module-dashboard-context` is present (upstream Dashboard Context Manager). Runtime enablement is unknown; accountable owner must select a replacement, retirement, or funded implementation.',now(),'legacy-module-inventory-seed'),
  ...
  ('EHI_EXPORTER','EHI Exporter','interoperability','decision-required','Legacy custom-module source `oe-module-ehi-exporter` is present (upstream Composer package oe-module-ehi-exporter). Runtime enablement is unknown; accountable owner must select the export/retention implementation.',now(),'legacy-module-inventory-seed'),
  ('FAX_SMS','Fax and SMS','communications','partner-gated','Legacy custom-module source `oe-module-faxsms` is present (upstream Composer package oe-module-faxsms). Runtime enablement is unknown; approved delivery-provider contract is required.',now(),'legacy-module-inventory-seed'),
