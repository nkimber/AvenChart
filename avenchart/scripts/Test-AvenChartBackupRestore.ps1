...
$ResultPath = Join-Path $ArtifactsRoot 'latest-modernized-backup-restore-rehearsal.json'
...
try {
    ...
    docker compose exec -T postgres psql -X -U legacy-ehr -d legacy-ehr_modernized -v ON_ERROR_STOP=1 -c 'drop table if exists backup_restore_rehearsal_marker;'
    ...
    & .\scripts\Backup-ModernizedDatabase.ps1 -OutputDirectory $runDirectory -PostgresWaitSeconds $PostgresWaitSeconds
    ...
    docker compose exec -T postgres psql -X -U legacy-ehr -d legacy-ehr_modernized -v ON_ERROR_STOP=1 -c "create table backup_restore_rehearsal_marker (id integer primary key, created_at timestamptz not null default now()); insert into backup_restore_rehearsal_marker (id) values (1);"
    ...
    & .\scripts\Restore-ModernizedDatabase.ps1 -SnapshotPath $backupManifest.backup -Force -PostgresWaitSeconds $PostgresWaitSeconds
