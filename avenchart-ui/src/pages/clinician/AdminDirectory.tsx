...
export default function AdminDirectory() {
  ...
  const [moduleHistory, setModuleHistory] = useState<ModuleCatalogHistory | null>(null)
  const [apiClients, setApiClients] = useState<ApiClientRegistryItem[]>([])
  const [apiClientHistory, setApiClientHistory] = useState<ApiClientRegistryHistory | null>(null)
  ...
  async function openModuleHistory(key: string) { try { setModuleHistory(await getModuleCatalogHistory(session.sessionId, key)) } catch { showToast('Could not load module history.', 'error') } }
  async function updateModuleStatus(module: ModuleCatalogItem, enabled: boolean) { try { const result = await updateModuleCatalogStatus(session.sessionId, module.key, enabled ? 'enabled' : 'disabled'); setModuleHistory(result); setModules((await getModuleCatalog(session.sessionId)).modules); showToast(`Module ${enabled ? 'enabled' : 'disabled'}.`, 'success') } catch { showToast('Could not update module status.', 'error') } }
  async function rollbackModuleRevision(revisionId: number) { if (!moduleHistory || !window.confirm(`Restore ${moduleHistory.module.displayName} to the selected historical state?`)) return; try { const result = await rollbackModuleCatalog(session.sessionId, moduleHistory.module.key, revisionId); setModuleHistory(result); setModules((await getModuleCatalog(session.sessionId)).modules); showToast('Module restored.', 'success') } catch { showToast('Could not restore the module.', 'error') } }
  async function openApiClientHistory(key: string) { try { setApiClientHistory(await getApiClientRegistryHistory(session.sessionId, key)) } catch { showToast('Could not load API-client history.', 'error') } }
  async function rollbackApiClientRevision(revisionId: number) { if (!apiClientHistory || !window.confirm(`Restore ${apiClientHistory.client.displayName} to the selected historical registration?`)) return; try { const result = await rollbackApiClientRegistry(session.sessionId, apiClientHistory.client.key, revisionId); setApiClientHistory(result); setApiClients((await getApiClients(session.sessionId)).clients); showToast('API client restored.', 'success') } catch { showToast('Could not restore the API client.', 'error') } }
