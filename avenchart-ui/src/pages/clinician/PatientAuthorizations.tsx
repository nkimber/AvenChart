...
export default function PatientAuthorizations() {
  ...
  const [approvalNumber, setApprovalNumber] = useState<Record<string, string>>(
    {},
  );
  ...
  async function create() {
    if (!payer.trim() || !service.trim()) return;
    try {
      await createPatientAuthorization(session.sessionId, patientId, {
        payer,
        service,
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "Unable to create authorization.",
      );
    }
  }
  async function transition(
    ...
    try {
      await updatePatientAuthorizationStatus(
        ...
        id,
        status,
        status === "approved" ? approvalNumber[id] : undefined,
      );
      setError("");
      load();
    } catch (reason) {
  ...
  return (
    <div className="clinician-page">
      <div className="clinician-page-header">
        <h1 className="clinician-page-title">Authorizations</h1>
        <p className="clinician-page-subtitle">
          Document local payer-authorization status; no payer submission or
      ...
      <section className="cl-card">
                ...
                <td>
                  {item.status === "draft" && (
                    <>
                      <button
                        ...
                        type="button"
                        onClick={() => transition(item.id, "submitted")}
                      >
                        Submit
                      </button>{" "}
                      <button
                  ...
                  {item.status === "submitted" && (
                    <>
                      ...
                      <button
                        className="cl-btn-secondary"
                        type="button"
                        onClick={() => transition(item.id, "denied")}
                      >
                        Deny
                      </button>
                    </>
                  ...
                  {item.status === "approved" && (
                    <button
                      className="cl-btn-secondary"
                      type="button"
                      onClick={() => transition(item.id, "expired")}
                    >
                      Expire
                    </button>
                  )}
        ...
        {authorizations.length === 0 && (
          <p className="cl-empty-text">No authorizations have been recorded.</p>
        )}
      </section>
    </div>
  );
